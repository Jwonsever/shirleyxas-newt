...
/* FIXME - this is probably not entirely safe (eg if split empty string) */
...
{
const gchar *energy_package=NULL, *energy_value=NULL, *energy_gradient=NULL;
const gchar *energy_method=NULL, *energy_basis=NULL, *energy_functional=NULL;
...
switch(model->id)
  ...
  case GULPOUT:
    energy_package = "GULP";
    energy_method = "Forcefield";
    energy_value = property_lookup("Energy", model);
    energy_gradient = property_lookup("Gnorm", model);
    break;
  case MVNOUT:
    energy_package = "MARVIN";
    energy_method = "Forcefield";
    energy_gradient = property_lookup("Gnorm", model);
    break;
  case NWCHEM_OUT:
    energy_package = "NWCHEM";
    energy_gradient = property_lookup("RMS Gradient", model);
    break;
  case SIESTA_OUT:
    energy_package = "SIESTA";
