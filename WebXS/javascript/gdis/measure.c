...
/*******************************************************************/
/* recompute all measurements and remove if components are deleted */
/*******************************************************************/
void measure_refresh(struct model_pak *model)
{
gint i, size, delete;
GSList *list, *next;
struct core_pak *core;
struct measure_pak *measure;
...
/* get number of cores comprising this measurement */
  switch (measure->type)
    {
    case MEASURE_ANGLE:
      size = 3;
...
/* if a core is flagged as deleted - remove measurement from list */
  delete = FALSE;
  for (i=size ; i-- ; )
    {
    core = measure->core[i];
