...
/*************************************/
/* create a text block for variables */
/*************************************/
// NWCHEM
gchar *zmatrix_connect_text(gpointer data)
{
gint i, n, m;
GSList *list;
GString *buff;
...
for (list=zmat->zlines ; list ; list=g_slist_next(list))
  ...
  g_string_append_printf(buff, "  %s", elements[zval->type].symbol);
  if (n > 3)
    m = 3;
  else
    m = n;
  for (i=0 ; i<m ; i++)
    {
    g_string_append_printf(buff, "  %d", zval->connect[i]);
    if (zval->name[i])
      g_string_append_printf(buff, "  %s", (gchar *) zval->name[i]);
    else
      g_string_append_printf(buff, "  %f", zval->value[i]);
    }
...
/*************************************/
// NWCHEM
gchar *zmatrix_constants_text(gpointer data)
{
gchar *value;
GList *item, *list;
GString *buff;
...
list = g_hash_table_get_keys(zmat->consts);
for (item=list ; item ; item=g_list_next(item))
  {
  value = g_hash_table_lookup(zmat->consts, item->data);
  g_string_append_printf(buff, "  %s  %s\n", (gchar *) item->data, value);
  }
...
/*************************************/
// NWCHEM
gchar *zmatrix_variables_text(gpointer data)
{
gchar *value;
GList *item, *list;
GString *buff;
...
list = g_hash_table_get_keys(zmat->vars);
for (item=list ; item ; item=g_list_next(item))
  {
  value = g_hash_table_lookup(zmat->vars, item->data);
  g_string_append_printf(buff, "  %s  %s\n", (gchar *) item->data, value);
  }
...
/*********************************/
#define DEBUG_ZMAT_NWCHEM_CORE_ADD 0
void zmat_nwchem_core_add(const gchar *line, gpointer data)
{
gint i, nc, num_tokens;
gchar **buff;
struct zmat_pak *zmat = data;
struct zval_pak *zval;
...
zval->type = elem_test(*buff);
zval->elem = g_strdup(elements[zval->type].symbol);
...
#if DEBUG_ZMAT_NWCHEM_CORE_ADD
printf("nc = %d\n", nc);
#endif

for (i=0 ; i<nc ; i++)
  {
  zval->connect[i] = str_to_float(*(buff+2*i+1));
  }
...
/* if it's a number -> value, else stored as variable name */
  if (str_is_float(*(buff+2*i+2)))
    zval->value[i] = str_to_float(*(buff+2*i+2));
  else
    zval->name[i] = g_strdup(*(buff+2*i+2));

#if DEBUG_ZMAT_NWCHEM_CORE_ADD
printf("[%s]", (gchar *) zval->name[i]);
#endif
  }
...
/* add a zmatrix coordinate line */
/*********************************/
// NB: this is specific to siesta's weird idea of a zmatrx
...
*/
/* CURRENT - this seems to be a (broken) siesta specific hack ... lets make it more general */
zval->type = elem_test(*(buff+1));
...
/* all the cores created below */
  switch (n)
    {
    case 0:
      core[0] = core_new(zval->elem, NULL, model);
    ...
    case 1:
      core[1] = core_new(zval->elem, NULL, model);
...
/* NWChem: this format has no values at this level of the zmatrix, */
/* so the defaults of 0.0 are used, which results in the z-axis */
/* being the direction along which the 2nd atom is translated */
...
/* NB: arbitrary choice of z for finding some orthogonal */
/* vector to do the bond angle rotation around */
      VEC3SET(v3, 0.0, 0.0, 1.0);
      crossprod(v1, v3, v2);
/* NWChem: because of the siesta hack, the crossprod above will be of 0 length */
/* so we just use the x axis as an alternative arbitrary direction */
      if (VEC3MAGSQ(v1) < POSITION_TOLERANCE)
        {
        VEC3SET(v3, 1.0, 0.0, 0.0);
        crossprod(v1, v3, v2);
        }
...
/* generate the atom position */
      core[2] = core_new(zval->elem, NULL, model);
...
/* generate the atom position */
      core[3] = core_new(zval->elem, NULL, model);
