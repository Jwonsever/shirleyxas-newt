...
/* Energy */
  ...
  if (g_strrstr(line, "Predicted change in Energy") != NULL)
    ...
    if (fgetline(fp, line))
      {
      gui_text_show(ERROR, "unexpected end of file checking for successful optimization\n");
      return(2);
      }
...
/* read coordinates */
  if ((g_strrstr(line, "Input orientation:") != NULL) || (g_strrstr(line, "Z-Matrix orientation:") != NULL)
   || (g_strrstr(line, "Standard orientation:") != NULL))
    {
