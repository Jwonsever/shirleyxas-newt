...
#define TIME_FORMAT_C "%d-%d-%d_%d:%d:%d"
...
/******************************************************/
const gchar *time_string(time_t *t1)
{
struct tm *t2;
static gchar text[30];

/* if NULL - get current time */
if (!t1)
  *t1 = time(NULL);

t2 = localtime(t1);

if (strftime(text, 30, TIME_FORMAT, t2))
  return(text);
...
/**********************************************/
/* compare two times in gdis timestamp format */
/**********************************************/
/* NB: time functions were doing weird stuff - tokenize and do it ourselves */
gint time_stamp_compare(const gchar *t1, const gchar *t2)
{
gint i, j, n1, n2;
gint v1[6], v2[6];
...
/* skip to 1st numeric char */
for (i=0 ; i<strlen(t1) ; i++)
  if (g_ascii_isdigit(t1[i]))
    break;
for (j=0 ; j<strlen(t2) ; j++)
  if (g_ascii_isdigit(t2[j]))
    break;

/* extract compare arrays (most to least significant) */
n1 = sscanf(&t1[i], TIME_FORMAT_C, &v1[2], &v1[1], &v1[0], &v1[3], &v1[4], &v1[5]);
n2 = sscanf(&t2[j], TIME_FORMAT_C, &v2[2], &v2[1], &v2[0], &v2[3], &v2[4], &v2[5]);
...
if (n1 == 6 && n2 == 6)
  ...
  {
// unexpected format, probably a grisu submitted jobname...
...
/* SINE replacement */
/********************/
#define EQUIV_DIV_2PI 0.159154943
#define EQUIV_MUL_2PI 6.283185307
...
/* enforce range [0, 2PI] */
idx = angle*EQUIV_DIV_2PI;
a = angle - idx*EQUIV_MUL_2PI;
/* -ve case */
if (a < 0.0)
  a += EQUIV_MUL_2PI;

/* init retrieval indices */
idx2 = idx = T_STEP_INV * a;
if (idx2)
  idx2--;
else
  idx2++;
