...
if (active_entry)
  ...
  if (model)
    {
// model_dup() does not add the model to the list, so do it now
sysenv.mal = g_slist_append(sysenv.mal, model);

    model_prep(model);
    gui_tree_append_model(entry->name, model);
    tree_select_model(model);
    }
  else
    gui_text_show(ERROR, "Failed to import library model.\n");
  }
}
...
GtkWidget *panel, *hbox, *vbox, *swin, *tree1, *tree2, *view;

g_assert(box != NULL);

/* frame with split pane (folders : entries) */
panel = gui_frame_vbox("Library", TRUE, TRUE, box);
...
gtk_box_pack_start(GTK_BOX(panel),vbox,FALSE,FALSE,PANEL_SPACING); 
