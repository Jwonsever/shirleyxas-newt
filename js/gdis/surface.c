...
/****************************/
/* free a surface structure */
/****************************/
void surface_free(gpointer data)
{
struct surface_pak *surface=data;

if (!surface)
  return;

plane_data_free(surface->planes);

g_free(surface);
}

/**********************************/
/* initialize a surface structure */
/**********************************/
void surface_init(gpointer data)
{
struct surface_pak *surface=data;

g_assert(surface != NULL);

surface->morph_type = DHKL;
surface->optimise = FALSE;
surface->converge_eatt = FALSE;
surface->converge_r1 = TRUE;
surface->converge_r2 = TRUE;
surface->include_polar = FALSE;
surface->ignore_bonding = FALSE;
surface->create_surface = FALSE;
surface->true_cell = FALSE;
surface->keep_atom_order = FALSE;
surface->ignore_symmetry = FALSE;
surface->bonds_full = 0;
surface->bonds_cut = 0;
surface->shift = 0.0;
surface->dspacing = 0.0;
surface->region[0] = 1;
surface->region[1] = 1;
surface->dipole_tolerance=0.005;
surface->totalenergy=0.0;
surface->esurf[0]=0.0;
surface->esurf[1]=0.0;
surface->eatt[0]=0.0;
surface->eatt[1]=0.0;
surface->gnorm=0.0;
VEC3SET(surface->miller, 1, 0, 0);
VEC3SET(surface->depth_vec, 0.0, 0.0, 0.0);
surface->planes=NULL;
surface->cwd=NULL;
}
...
/******************************/
gpointer surface_new(void)
{
struct surface_pak *surface;

surface = g_malloc(sizeof(struct surface_pak));
...
/********************************************/
/* if no model - just create withough any symmetry calcs */
...
if (len > FRACTION_TOLERANCE)
  ...
  {
  printf("Error: plane of zero length created.\n");
  VEC3SET(plane->x, 0.0, 0.0, 0.0);
  }
...
/************************************************************/
gpointer surface_hkl_find(gdouble *hkl, GSList *list, struct model_pak *model)
{
GSList *item;
struct plane_pak *comp, *plane;

//P3VEC("Looking for: ", hkl);
...
#define DEBUG_SURF_SYMMETRY_GENERATE 1
void surface_symmetry_generate(struct surface_pak *surface, struct model_pak *model)
{
gint i;
gint *sym_hkl;
gdouble m[3];
...
surface_print_planes(surface->planes);
#endif

/* compute symmetry related faces */
new_planes = NULL;
//for (list=model->planes ; list ; list=g_slist_next(list))
for (list=surface->planes ; list ; list=g_slist_next(list))
  {
  plane = list->data;
...
// TODO - use zones to avoid creating duplicates in the first place
...
if (!pdata->index[0] && !pdata->index[1] && !pdata->index[2])
  {
  gui_text_show(ERROR, "Zero length plane not permitted.\n");
  return(NULL);
  }
...
/* label it as MARVIN, so it's build mode follows the */
...
if (fabs(sbe) < FRACTION_TOLERANCE)
  {
  gui_text_show(WARNING, "Unit cell energy uncalculated? Surface energy calculations may be suspect.\n");
  }
...
/* graft to GUI tree*/
name = g_strdup_printf("surface_%d%d%d_%.3f", pdata->index[0], pdata->index[1], pdata->index[2], sdata->shift);
#ifdef WITH_GUI
gui_tree_append_model(name, surf);
#endif
g_free(name);
...
void update_plane_energy(struct plane_pak *plane, struct surface_pak *surface)
{
gint i;
GSList *list;
