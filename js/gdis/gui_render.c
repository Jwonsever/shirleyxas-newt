...
gdouble render_animate_frames = 10;
...
/* NEW - refresh primitive drawing pre-calcs */
...
gui_button_x("CPK with bonds", render_mode_set, GINT_TO_POINTER(CPK_STICK), vbox);
...
/* NEW - auto set to white background if grayscale */
/* NB: will lose any non black background when switching back, */
/* but this is what the people want */
if (g_ascii_strncasecmp(tmp, "gray", 4) == 0)
  {
  model->grayscale = TRUE;
  VEC3SET(sysenv.render.bg_colour, 1.0,1.0,1.0);
  }
else
  {
...
list = g_list_append(list, "grayscale");
...
g_signal_connect(GTK_OBJECT(entry), "changed", (gpointer) set_colour_scheme, entry);
...
gui_button_x("Ball and Stick", render_mode_set, GINT_TO_POINTER(BALL_STICK), vbox);
gui_button_x("CPK", render_mode_set, GINT_TO_POINTER(CPK), vbox);
gui_button_x("CPK with bonds", render_mode_set, GINT_TO_POINTER(CPK_STICK), vbox);
gui_button_x("Liquorice", render_mode_set, GINT_TO_POINTER(LIQUORICE), vbox);
gui_button_x("Polyhedral", render_mode_polyhedral, NULL,  vbox);
...
gui_direct_spin("Ball radius",
                  ...
                  gui_render_refresh, NULL, vbox);

gui_direct_spin("Bond radius",
...
/***********************************/
void gui_delete_waypoint(GtkWidget *w, gpointer dummy)
{
GtkTreeIter iter;
...
/* checks */
if (GTK_IS_TREE_VIEW(camera_tree))
...
if (gtk_tree_selection_get_selected(selection, &treemodel, &iter))
  ...
  if (camera == model->camera_default)
    {
    printf("You cannot delete the default camera.\n");
    return;
    }
  else
    {
...
gui_button_x("Delete waypoint ", gui_delete_waypoint, NULL, vbox);
...
/*********************************/
/* surface material highlighting */
/*********************************/
void gui_render_highlight_section(GtkWidget *box)
{
GtkWidget *vbox;
...
/* surface look */
gui_render_highlight_section(vbox1);
...
gui_auto_check("Show axes", gui_render_refresh, NULL, &model->show_axes, vbox);
gui_auto_check("Show cell", gui_render_refresh, NULL, &model->show_cell, vbox);
gui_auto_check("Show cell images", gui_render_refresh, NULL, &model->show_cell_images, vbox);
gui_auto_check("Show cell lengths", gui_render_refresh, NULL, &model->show_cell_lengths, vbox);
// FIXME - doesn't work
//gui_auto_check("Show cores", gui_render_refresh, NULL, &model->show_cores, vbox);
gui_auto_check("Show core-shell links", gui_render_refresh, NULL, &model->show_links, vbox);
gui_auto_check("Show core index", gui_render_refresh, NULL, &model->show_atom_index, vbox);
gui_auto_check("Show core label", gui_render_refresh, NULL, &model->show_atom_labels, vbox);
gui_auto_check("Show core type", gui_render_refresh, NULL, &model->show_atom_types, vbox);
gui_auto_check("Show core charge", gui_render_refresh, NULL, &model->show_atom_charges, vbox);

/*VZ*/
gui_auto_check("Show NMR shielding", gui_render_refresh, NULL, &model->show_nmr_shifts, vbox);
gui_auto_check("Show NMR CSA", gui_render_refresh, NULL, &model->show_nmr_csa, vbox);
gui_auto_check("Show NMR EFG", gui_render_refresh, NULL, &model->show_nmr_efg, vbox);
...
gui_auto_check("Show total charge", gui_render_refresh, NULL, &model->show_charge, vbox);

gui_auto_check("Show labels on selection ", gui_render_refresh, NULL, &model->show_selection_labels, vbox);
gui_auto_check("Show shells", gui_render_refresh, NULL, &model->show_shells, vbox);

gui_auto_check("Show camera waypoints ", morph_toggle, NULL, &model->show_waypoints, vbox);

/* next frame */
//vbox = gui_frame_vbox(NULL, FALSE, FALSE, vbox2);
//gui_direct_check("Show energy", &sysenv.render.show_energy, gui_render_refresh, NULL, vbox);
// FIXME - doesnt work
...
/* POVRay page */
/* moved to the new animation/rendering dialog */
/*
page = gtk_vbox_new(FALSE, PANEL_SPACING);
...
/* terminating buttons */
gui_stock_button(GTK_STOCK_HELP, gui_help_show, "Display Properties", GTK_DIALOG(window)->action_area);
