...
/* CURRENT - we seem to have to do something special here to generate 2 redraws */
/* not sure why - this in combination with the subsequent expose will give us */
/* the 2 redraw requests we need */
sysenv.refresh_canvas=1;
...
{
//printf("canvas_expose()\n");

/* combined with a configure - we get 2 requests (see above) */
...
if (!gtk_widget_set_gl_capability(sysenv.glarea, sysenv.glconfig, NULL, TRUE, GDK_GL_RGBA_TYPE))
  printf("canvas_init(): failed to make an OpenGL canvas.\n");
...
/* NEW - turn auto double buffering off as we do it ourselves */
GTK_WIDGET_UNSET_FLAGS(sysenv.glarea, GTK_DOUBLE_BUFFERED);
...
/* only select if not already active -avoid's deselecting graphs */
        if (canvas->model != tree_model_get())
