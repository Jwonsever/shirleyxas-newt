...
/**************************************/
/* add a core if it's not a duplicate */
/**************************************/
gint space_fill_core(struct core_pak *core1, gpointer za)
{
gdouble r[3];
gpointer zone;
GSList *item, *list;
...
zone = zone_get(core1->x, za);
list = zone_cores(zone);
for (item=list ; item ; item=g_slist_next(item))
  ...
  ARR3SUB(r, core2->x);
  fractional_min(r, 3);
  if (VEC3MAGSQ(r) < FRACTION_TOLERANCE)
    return(TRUE);
  }

/* update zone info */
zone_core_add(core1, zone);
...
/* KR. Rewrite to avoid order-dependency of loop which caused 
   bug - missing images for some cif files which contain
   matrices where identity is not first. */
/*  for (j=(s+1)/2 ; j<model->sginfo.order ; j++)*/
   for (j=0 ; j<model->sginfo.order ; j++)
     {
       if ( ! matrix_is_identity(*(model->sginfo.matrix+j)))
	 {
...
/* NB: this adds the core to the zone list */
if (space_fill_core(sr_core, za))
  {
  core_free(sr_core);
  continue;
  }
