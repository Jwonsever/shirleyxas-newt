...
/* point of intersection of n1,n2,n3 - returned in x (if exists) */
...
gint morph_setup(gint m, gint **hkl, gdouble **mat, struct surface_pak *surface)
{
gint i, n;
gdouble max, value;
GSList *list;
...
/* generate hkl label */
    plane = g_slist_nth_data(surface->planes, i);
    if (plane)
      spatial->label = g_strdup_printf("(%d%d%d)", hkl[i][0], hkl[i][1], hkl[i][2]);

/* CURRENT - save the plane within the model */
    copy = plane_dup(plane);
    model->planes = g_slist_prepend(model->planes, copy);
    }
  else
    absent[i] = TRUE;
  }
