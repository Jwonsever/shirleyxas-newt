...
if (tree_selected_get(&id, &label, &data))
  ...
  switch (id)
    ...
    default:
      gui_text_show(ERROR, "Can't copy that object type.\n");
    }
  }
#endif
}
...
/* checks */
if (!tree_selected_get(&paste_id, &label, &paste_data))
  {
  gui_text_show(ERROR, "No destination for paste operation.\n");
  return;
  }
if (!gui_tree_grafted(&src_id, sysenv.select_source))
  {
  gui_text_show(ERROR, "No source data for paste operation.\n");
  sysenv.select_source = NULL;
  return;
  }
if (src_id != paste_id)
  {
  gui_text_show(ERROR, "Source and destination objects don't match.\n");
  return;
  }
...
/* updates */
  ...
  case TREE_GRAPH:
    graph_overlay(paste_data, sysenv.select_source);
    redraw_canvas(SINGLE);
    break;
...
/*****************************/
void cb_select_colour(GtkWidget *w, gpointer csd)
{
GSList *list;
GdkColor colour;
...
{
#ifdef WITH_GUI
gui_colour_selection_dialog("Edit Colour", cb_select_colour);
#endif
}

/********************************/
/* undo deletion of a selection */
/********************************/
#define DEBUG_SELECT_UNDO_DELETE 0
void select_undo_delete(struct model_pak *model, gpointer data)
{
GSList *item, *list=data;
struct core_pak *core;
...
#if DEBUG_SELECT_UNDO_DELETE
printf("undo delete: %d cores\n", g_slist_length(list));
#endif

model->cores = g_slist_concat(model->cores, list);

/* add assoc shells (if any) and remove deletion flag */
...
/* recalc connectivity and drawing primitives */
connect_refresh(model);
render_refresh(model);
}

/********************/
/* delete selection */
/********************/
#define DEBUG_SELECT_DELETE 0
...
#if DEBUG_SELECT_DELETE
printf("# cores = %d\n", g_slist_length(model->cores));
#endif

/* flag the core as deleted, but dont remove it yet due to dependencies */
for (list=model->selection ; list ; list=g_slist_next(list))
  {
  delete_core(list->data);
  }

/* remove all traces of the core from other lists */
connect_refresh(model);
render_refresh(model);
measure_refresh(model);

/* remove and free the core from the primary lists */
for (next=model->selection ; next ; next=g_slist_next(next))
  {
  core = next->data;
  if (core->shell) 
    {
    struct shel_pak *shell = core->shell;
    model->shels = g_slist_remove(model->shels, shell);
    }
  model->cores = g_slist_remove(model->cores, core);
  }


/* CURRENT - register undo */
undo_register(model, select_undo_delete, model->selection);
...
/***********************************/
/* add hydrogens to selected atoms */
/***********************************/
void select_add_hydrogens(struct model_pak *model)
{
GSList *item;

for (item=model->selection ; item ; item=g_slist_next(item))
  {
  edit_vsum_add_core("H", item->data, model);
  }
