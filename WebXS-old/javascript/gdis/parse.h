...
gchar *parse_nth_token_dup(gint, const gchar *);

gchar *parse_getline_hidden(void);

gint parse_time_hms(const gchar *);

gpointer parse_new(void);
void parse_free(gpointer);
void parse_keyword_add(const gchar *, gint, gpointer, gpointer);
gint parse_line(const gchar *, gpointer);
gchar *parse_inverse(gpointer);

/* parse processing types */
/* only process 1st token into character, all tokens (strjoinv) etc */
enum { PARSE_FIRST_CHAR, PARSE_ALL_CHAR };
