...
gulp->switch_type = GNORM;
gulp->switch_value = NULL;
...
gulp->solvation_model = NULL;
gulp->cosmo_shape = NULL;
gulp->cosmo_smoothing = NULL;
gulp->cosmo_solvent_epsilon = NULL;
gulp->cosmo_solvent_radius = NULL;
gulp->cosmo_solvent_rmax = NULL;
...
/*************************/
/* free a gulp structure */
/*************************/
void gulp_free(gpointer data)
{
struct gulp_pak *gulp=data;
...
g_free(gulp->potentials);
g_free(gulp->elements);
g_free(gulp->species);
g_free(gulp->kpoints);
g_free(gulp->epot_vecs);
g_free(gulp->pressure);
g_free(gulp->temperature);
g_free(gulp->libfile);
g_free(gulp->extra_keywords);
g_free(gulp->extra);
g_free(gulp->timestep);
g_free(gulp->equilibration);
g_free(gulp->production);
g_free(gulp->sample);
g_free(gulp->write);
g_free(gulp->temp_file);
g_free(gulp->dump_file);
g_free(gulp->out_file);
g_free(gulp->trj_file);
g_free(gulp->mov_file);
g_free(gulp->rigid_move);

g_free(gulp);
}

/*******************************************/
/* allocation and initialization primitive */
/*******************************************/
gpointer gulp_new(void)
{
struct gulp_pak *gulp;

gulp = g_malloc(sizeof(struct gulp_pak));
gulp_init(gulp);
...
/* octahedron is default */
/* see gulp manual @@pointsperatom */
c = 4.0;
if (gulp->cosmo_shape)
  {
  if (g_strncasecmp(gulp->cosmo_shape, "dodec", 5) == 0)
    c = 10.0;
  }
...
/*****************************/
void write_core(GString *buffer, struct core_pak *core, struct model_pak *model)
{
gint i;
gdouble vec[3];
...
if (core->breathe)
  g_string_append_printf(buffer,"%-4s bcore %11.6f %11.6f %11.6f", core->atom_label,
                                                vec[0], vec[1], vec[2]);
else
  g_string_append_printf(buffer,"%-4s core  %11.6f %11.6f %11.6f", core->atom_label,
                                                vec[0], vec[1], vec[2]);

/* NB: 7 dp for charges - 8 occasionally introduced small errors (cf GULP) */
if (model->gulp.print_charge)
  if (!core->lookup_charge)
    g_string_append_printf(buffer,"  %11.7lf", core->charge);

/* 7dp for occupancy - to cover cases like 1/3 */
if (core->has_sof)
  g_string_append_printf(buffer,"  %9.7f", core->sof);

if (core->radius != 0.0)
  g_string_append_printf(buffer,"  %7.5f", core->radius);

if (core->flags)
  g_string_append_printf(buffer," %s", core->flags);
...
if (core->translate)
  g_string_append_printf(buffer, " T");
...
/******************************/
void write_shell(GString *buffer, struct shel_pak *shell, struct model_pak *model)
{
gint i;
gdouble vec[3];
...
if (shell->breathe)
  g_string_append_printf(buffer,"%-4s bshel %11.6f %11.6f %11.6f", shell->shell_label,
                                                vec[0], vec[1], vec[2]);
else
  g_string_append_printf(buffer,"%-4s shel  %11.6f %11.6f %11.6f", shell->shell_label,
                                                vec[0], vec[1], vec[2]);

/* only print specific atom charges if we read them in */
if (model->gulp.print_charge)
  if (!shell->lookup_charge)
    g_string_append_printf(buffer,"  %11.7lf", shell->charge);

/* 7dp for occupancy - to cover cases like 1/3 */
if (shell->has_sof)
  g_string_append_printf(buffer,"  %9.7f", shell->sof);

if (shell->radius != 0.0)
  g_string_append_printf(buffer,"  %7.5f", shell->radius);

if (shell->flags)
  g_string_append_printf(buffer," %s", shell->flags);
...
if (src->gulp.switch_value)
  dest->gulp.switch_value = g_strdup(src->gulp.switch_value);
else
  dest->gulp.switch_value = NULL;
...
if (src->gulp.maxcyc)
  dest->gulp.maxcyc = g_strdup(src->gulp.maxcyc);
else
  dest->gulp.maxcyc = NULL;
...
if (src->gulp.solvation_model)
  {
  g_free(dest->gulp.solvation_model);
  dest->gulp.solvation_model = g_strdup(src->gulp.solvation_model);
  }
if (src->gulp.cosmo_shape)
  {
  g_free(dest->gulp.cosmo_shape);
  dest->gulp.cosmo_shape = g_strdup(src->gulp.cosmo_shape);
  }
if (src->gulp.cosmo_smoothing)
  {
  g_free(dest->gulp.cosmo_smoothing);
  dest->gulp.cosmo_smoothing = g_strdup(src->gulp.cosmo_smoothing);
  }
if (src->gulp.cosmo_solvent_epsilon)
  {
  g_free(dest->gulp.cosmo_solvent_epsilon);
  dest->gulp.cosmo_solvent_epsilon = g_strdup(src->gulp.cosmo_solvent_epsilon);
  }
if (src->gulp.cosmo_solvent_radius)
  {
  g_free(dest->gulp.cosmo_solvent_radius);
  dest->gulp.cosmo_solvent_radius = g_strdup(src->gulp.cosmo_solvent_radius);
  }
if (src->gulp.cosmo_solvent_rmax)
  {
  g_free(dest->gulp.cosmo_solvent_rmax);
  dest->gulp.cosmo_solvent_rmax = g_strdup(src->gulp.cosmo_solvent_rmax);
  }
...
/**********************************/
void write_region_header(GString *buffer, gint region, struct model_pak *model, struct gulp_pak *gulp)
{
/* check periodicity */
switch (model->periodic)
  ...
  case 1:
    if (model->fractional)
      g_string_append_printf(buffer,"pfractional region %d", region+1);
    else
      g_string_append_printf(buffer,"cartesian region %d", region+1);
    break;

  case 2:
    if (model->fractional)
      g_string_append_printf(buffer,"sfractional region %d", region+1);
    else
      g_string_append_printf(buffer,"cartesian region %d", region+1);
    break;
  ...
  case 3:
    if (model->fractional)
      g_string_append_printf(buffer,"fractional");
    else
      g_string_append_printf(buffer,"cartesian");
    break;
...
if (gulp->rigid && region == 2)
  {
  g_string_append_printf(buffer," rigid ");
  if (gulp->rigid_move)
    g_string_append_printf(buffer,"%s ", gulp->rigid_move);
  }
...
gint gulp_input_export(gpointer import)
{
...
struct gulp_pak *gulp, gulp_default;
...
gulp = import_config_data_get(GULP, import);
if (!gulp)
  {
  gulp_init(&gulp_default);
  gulp = &gulp_default;
  }
...
if (gulp->libfile)
  {
  if (strlen(gulp->libfile))
    {
/* test for library existance */
    if (!access(gulp->libfile, R_OK))
      libflag++;
    else
...
/* CURRENT - solvation calc */
if (gulp->solvation_model)
  g_string_append_printf(line, " %s", gulp->solvation_model);
...
if (g_slist_length(gulp->epot_vecs))
  g_string_append(line," epot");

/* print out the keywords line */
if (gulp->output_extra_keywords && gulp->extra_keywords)
  g_string_append_printf(buffer,"%s %s\n\n", line->str, gulp->extra_keywords);
else
...
/* NB: this surface data is only meaningful if generate from within GDIS */
  if (VEC3MAGSQ(model->surface.depth_vec) > FRACTION_TOLERANCE)
    {
    g_string_append_printf(buffer, "#       miller: %d %d %d\n",
                     (gint) model->surface.miller[0],
                     (gint) model->surface.miller[1],
                     (gint) model->surface.miller[2]);
    g_string_append_printf(buffer, "#        shift: %lf\n", model->surface.shift);
    g_string_append_printf(buffer, "# region sizes: %d %d\n\n",
...
/* optional data */
if (gulp->maxcyc)
  g_string_append_printf(buffer,"maxcyc %s\n\n", gulp->maxcyc);

if (gulp->optimiser2 != SWITCH_OFF)
  {
  g_string_append_printf(buffer,"switch ");
  switch(gulp->optimiser2)
    {
    case CONJ_OPT:
      g_string_append_printf(buffer,"conj ");
      break;
    case RFO_OPT:
      g_string_append_printf(buffer,"rfo ");
      break;
    case BFGS_OPT:
    default:
      g_string_append_printf(buffer,"bfgs ");
      break;
    }

  if (gulp->switch_value)
    {
    switch(gulp->switch_type)
      {
      case CYCLE:
        g_string_append_printf(buffer,"cycle %s\n\n", gulp->switch_value);
        break;
      case GNORM:
        g_string_append_printf(buffer,"gnorm %s\n\n", gulp->switch_value);
        break;
      }
    }
  }
...
if (libflag)
  g_string_append_printf(buffer,"library %s\n\n", gulp->libfile);

g_string_append_printf(buffer,"name %s\n\n", model->basename);

if (gulp->dump_file)
  if (strlen(gulp->dump_file))
    g_string_append_printf(buffer,"\ndump %s\n\n", gulp->dump_file);

if (gulp->temperature)
  if (strlen(gulp->temperature))
    g_string_append_printf(buffer,"temperature %s\n\n", gulp->temperature);

if (gulp->pressure)
  if (strlen(gulp->pressure))
    g_string_append_printf(buffer,"pressure %s\n\n", gulp->pressure);

/* dynamics stuff */
if (gulp->run == MD)
  {
  switch (gulp->ensemble)
    {
    case NVE:
      g_string_append_printf(buffer,"ensemble nve\n\n");
      break;
    case NVT:
      g_string_append_printf(buffer,"ensemble nvt\n\n");
      break;
    case NPT:
      g_string_append_printf(buffer,"ensemble npt\n\n");
      break;
    }

  if (gulp->timestep)
    g_string_append_printf(buffer, "timestep %s\n", gulp->timestep);
  if (gulp->equilibration)
    g_string_append_printf(buffer, "equilibration %s\n", gulp->equilibration);
  if (gulp->production)
    g_string_append_printf(buffer, "production %s\n", gulp->production);
  if (gulp->sample)
    g_string_append_printf(buffer, "sample %s\n", gulp->sample);
  if (gulp->write)
    g_string_append_printf(buffer, "write %s\n", gulp->write);

  g_string_append_printf(buffer, "\n");

  if (gulp->trj_file)
    g_string_append_printf(buffer, "output trajectory %s\n", gulp->trj_file);
  }

if (gulp->mov_file)
  {
  g_string_append_printf(buffer, "output movie arc %s\n", gulp->mov_file);
  if (gulp->run == MD)
    g_string_append_printf(buffer, "mdarchive %s\n", gulp->mov_file);
  }
...
if (gulp->solvation_model)
  {
  if (gulp->cosmo_shape)
    g_string_append_printf(buffer, "cosmoshape %s\n", gulp->cosmo_shape);
  else
    g_string_append_printf(buffer, "cosmoshape octahedron\n");

  g_string_append_printf(buffer, "pointsperatom %d\n", gulp->cosmo_points);
  g_string_append_printf(buffer, "segmentsperatom %d\n", gulp->cosmo_segments);

  if (gulp->cosmo_solvent_epsilon)
    g_string_append_printf(buffer, "solventepsilon %s\n", gulp->cosmo_solvent_epsilon);

  if (gulp->cosmo_solvent_radius)
    g_string_append_printf(buffer, "solventradius %s\n", gulp->cosmo_solvent_radius);

  if (gulp->cosmo_solvent_rmax)
    g_string_append_printf(buffer, "solventrmax %s\n", gulp->cosmo_solvent_rmax);

  if (gulp->cosmo_smoothing)
    g_string_append_printf(buffer, "rangeforsmooth %s\n", gulp->cosmo_smoothing);

  g_string_append_printf(buffer, "\n");

  if (gulp->cosmo_sas)
    g_string_append_printf(buffer, "output sas %s.sas\n\n", model->basename);
  }
...
/* NB: gulp matrices are transposed wrt gdis */
      g_string_append_printf(buffer,"vectors\n");
      ...
      {
      g_string_append_printf(buffer,"cell\n");
      g_string_append_printf(buffer,"%lf %lf %lf  %lf %lf %lf\n",model->pbc[0],model->pbc[1],model->pbc[2],
                model->pbc[3]*180/PI, model->pbc[4]*180/PI, model->pbc[5]*180/PI);
      }
    break;
  ...
  case 2:
    if (model->construct_pbc)
      {
      g_string_append_printf(buffer,"svectors\n");
      g_string_append_printf(buffer,"%lf  %lf\n%lf  %lf\n",model->latmat[0], model->latmat[3],
                                    model->latmat[1], model->latmat[4]);
      }
    else
      {
      g_string_append_printf(buffer,"scell\n");
      g_string_append_printf(buffer,"%lf %lf %lf\n",model->pbc[0], model->pbc[1],
  ...
  case 1:
    if (model->construct_pbc)
      {
      g_string_append_printf(buffer,"pvector\n");
      g_string_append_printf(buffer,"%lf\n",model->latmat[0]);
      }
    else
      {
      g_string_append_printf(buffer,"pcell\n");
      g_string_append_printf(buffer,"%lf\n",model->pbc[0]);
      }
    break;
  }
...
/* write suitable header if it hasn't yet been written */
    if (!flag)
      {
      write_region_header(buffer, region, model, gulp);
      flag++;
      }
    write_core(buffer, core, model);
    }
...
/* shells */
  for (list=model->shels ; list ; list=g_slist_next(list))
    ...
    write_shell(buffer, shell, model);
    }
  flag = 0;
  }
...
/* write dhkl for Eatt calcs */
    if (model->surface.dspacing > 0.1 && !gulp->no_eatt)
      g_string_append_printf(buffer, "\ndhkl %lf\n",model->surface.dspacing);
...
/* SG info */
  case 3:
    if (model->sginfo.spacename)
      {
      g_string_append_printf(buffer,"\nspace\n%s\n",g_strstrip(model->sginfo.spacename));
      if (model->sginfo.cellchoice)
        g_string_append_printf(buffer,"\norigin %d\n",model->sginfo.cellchoice);
      }
    else
      {
      g_string_append_printf(buffer,"\nspace\nP 1\n");
...
if (gulp->noautobond)
  ...
  for (list=model->bonds ; list ; list=g_slist_next(list))
    ...
    i = 1+g_slist_index(model->cores, core1);
    j = 1+g_slist_index(model->cores, core2);

/* connect i j <type> ix iy iz */
    g_string_append_printf(buffer, "connect %4d %4d", i, j);

    switch(bond->type)
      {
      case BOND_TRIPLE:
        g_string_append_printf(buffer, "  triple");
        break;
      case BOND_DOUBLE:
        g_string_append_printf(buffer, "  double");
        break;
      case BOND_SINGLE:
      default:
        g_string_append_printf(buffer, "  single");
        break;
      }
...
if (gulp->species)
  {
  strip_extra(gulp->species);
  if (strlen(gulp->species))
    {
    g_string_append_printf(buffer,"species\n");
    g_string_append_printf(buffer,"%s\n", gulp->species);
...
if (gulp->elements)
  {
  strip_extra(gulp->elements);
  if (strlen(gulp->elements))
    {
    g_string_append_printf(buffer,"elements\n");
    g_string_append_printf(buffer,"%s\n", gulp->elements);
    g_string_append_printf(buffer,"end\n\n");
    }
  }
...
if (gulp->epot_vecs)
  {
  g_string_append_printf(buffer, "potsites cart\n");

  for (list=gulp->epot_vecs ; list ; list=g_slist_next(list))
    {
    vec = list->data;
...
/* specified kpoints? */
if (model->periodic && gulp->kpoints)
  {
  strip_extra(gulp->kpoints);
  if (strlen(gulp->kpoints))
    {
    g_string_append_printf(buffer, "kpoints %d\n", 1+char_count(gulp->kpoints, '\n'));
    g_string_append_printf(buffer, "%s\n\n", gulp->kpoints);
    }
  }

/* unrecognized stuff */
if (gulp->output_extra)
  {
  if (gulp->extra)
    {
...
g_string_append_printf(buffer, "print 1\n");
...
gint gulp_change_model(gint trigger)
{
...
/* yet another gulp read rewrite */
/*********************************/
#define DEBUG_GULP_IMPORT 0
gint gulp_input_import(gpointer import)
{
gint i, j, nc, ns;
...
gchar *filename, *tmp, *line, **buff;
gdouble x[3], vec1[3], vec2[3], vec3[3];
gpointer scan, config;
GSList *item, *clist, *slist, *model_list=NULL;
...
#if DEBUG_GULP_IMPORT
printf("importing: %s\n", filename);
#endif

/* init alloc */
model = model_new();
model_list = g_slist_prepend(model_list, model);
gulp = gulp_new();
...
noflags = FALSE;
region = REGION1A;
gulp_change_model(GULP_NEW_FLAGS);
...
/* set flag incase we have more than one structure in the file */
        ...
        case GULP_SOLVATION_COSMO:
          gulp->solvation_model = g_strdup(*(buff+i));
          keywords_found++;
          keywords_expected = num_tokens;
          break;
...
/* compute the bond offset (midpoint vector in fractionals) */
        ...
        case GULP_COSMO_SHAPE:
          if (num_tokens > 1)
            gulp->cosmo_shape = g_strdup(*(buff+1));
          break;
        ...
        case GULP_COSMO_SMOOTHING:
          if (num_tokens > 1)
            gulp->cosmo_smoothing = g_strdup(*(buff+1));
          break;
        case GULP_COSMO_SOLVENT_EPSILON:
          if (num_tokens > 1)
            gulp->cosmo_solvent_epsilon = g_strdup(*(buff+1));
          break;
        case GULP_COSMO_SOLVENT_RADIUS:
          if (num_tokens > 1)
            gulp->cosmo_solvent_radius = g_strdup(*(buff+1));
          break;
        case GULP_COSMO_SOLVENT_RMAX:
          if (num_tokens > 1)
            gulp->cosmo_solvent_rmax = g_strdup(*(buff+1));
          break;
        ...
        case SWITCH_ON:
          for (j=1 ; j<num_tokens ; j++)
            ...
            switch (code)
              ...
              case CONJ_OPT:
              case RFO_OPT:
                gulp->optimiser2 = code;
                break;

              case GNORM:
              case CYCLE:
                gulp->switch_type = code;
                break;
 
              default:
                gulp->switch_value = g_strdup(*(buff+j));
              }
            }
          break;
...
/* NB: these keywords can initiate a new model (ie if repeat occurs) */
        ...
        case CART:
          if (num_tokens > 2)
            ...
            if (region == REGION1A)
              {
              if (gulp_change_model(GULP_NEW_COORDS))
                {
                model = model_new();
                model_list = g_slist_prepend(model_list, model);
                }
...
/* another keyword that can initiate a new model (on repeat) */
        case NAME:
          if (gulp_change_model(GULP_NEW_NAME))
            {
            model = model_new();
            model_list = g_slist_prepend(model_list, model);
            }
...
/* other keywords that can initiate a new model (on repeat) */
        ...
        case SURFACE_VECTORS:
        case LATTICE_VECTORS:
          if (gulp_change_model(GULP_NEW_LATTICE))
            {
#if DEBUG_GULP_IMPORT
printf("new model created on: %s\n", scan_cur_line(scan));
#endif
            model = model_new();
            model_list = g_slist_prepend(model_list, model);
            }
      ...
      case OUTPUT:
        ...
        case TEMPERATURE:
          if (num_tokens > 1)
            {
            g_free(gulp->temperature);
            gulp->temperature = g_strjoinv(" ", buff+1);
            }
          else
            {
/* get next (non empty) line */
            ...
            if (buff)
              {
              g_free(gulp->temperature);
              gulp->temperature = g_strjoinv(" ", buff);
              }
            }
          context = -1;
          break;
        ...
        case PRESSURE:
          if (num_tokens > 1)
            {
            g_free(gulp->pressure);
            gulp->pressure = g_strjoinv(" ", buff+1);
            }
          else
            {
/* get next (non empty) line */
       ...
       case TIMESTEP:
         g_free(gulp->timestep);
         gulp->timestep = g_strdup(get_token_pos(scan_cur_line(scan), 1));
         g_strstrip(gulp->timestep);
         break;
       case EQUILIBRATION:
         g_free(gulp->equilibration);
         gulp->equilibration = g_strdup(get_token_pos(scan_cur_line(scan), 1));
         g_strstrip(gulp->equilibration);
         break;
       case PRODUCTION:
         g_free(gulp->production);
         gulp->production = g_strdup(get_token_pos(scan_cur_line(scan), 1));
         g_strstrip(gulp->production);
         break;
       case SAMPLE:
         g_free(gulp->sample);
         gulp->sample = g_strdup(get_token_pos(scan_cur_line(scan), 1));
         g_strstrip(gulp->sample);
         break;
       case WRITE:
         g_free(gulp->write);
         gulp->write = g_strdup(get_token_pos(scan_cur_line(scan), 1));
         g_strstrip(gulp->write);
         break;

        case SUPER_CELL:
          if (num_tokens > 3)
            {
            gulp->super[0] = str_to_float(*(buff+1));
            gulp->super[1] = str_to_float(*(buff+2));
            gulp->super[2] = str_to_float(*(buff+3));
            }
          break;
        ...
        case NAME:
          if (num_tokens > 1)
            ...
            model->filename = g_strdup(model->basename);
            }
          break;
...
/* new core */
        ...
        case SBULK_ENERGY:
          if (num_tokens > 1)
            gulp->sbulkenergy = str_to_float(*(buff+1));
          context = -1;
          break;

        case TOTAL_ENERGY:
          if (num_tokens > 1)
            gulp->energy = str_to_float(*(buff+1));
          context = -1;
          break;
...
/* NEW - grab everything except the dump option itself */
            g_free(gulp->dump_file);
            line = scan_cur_line(scan);
            gulp->dump_file = g_strdup(get_token_pos(line,1));
            strip_extra(gulp->dump_file);
            }
          context = -1;
          break;
...
/* get buffered data data */
gulp->kpoints = g_string_free(kpoints_buff, FALSE);
gulp->potentials = g_string_free(ff_buff, FALSE);
gulp->species = g_string_free(species_buff, FALSE);
gulp->elements = g_string_free(elem_buff, FALSE);
gulp->extra = g_string_free(ex_buff, FALSE);
gulp->extra_keywords = g_string_free(key_buff, FALSE);


/* NEW */
config = config_new(GULP, gulp);
import_object_add(IMPORT_CONFIG, config, import);


/* setup newly loaded models */
for (item=model_list ; item ; item=g_slist_next(item))
  {
  model = item->data;
...
/* connectivity */
  ...
  buff = tokenize(gulp->species, &num_tokens);
  i = 0;
  while (i < num_tokens)
    {
...
/* process element data */
  buff = tokenize(gulp->elements, &num_tokens);
  i = 0;
  type = -1;
  while (i < num_tokens)
    {
...
/*
    gulp->orig_fractional = model->fractional;
    gulp->orig_construct_pbc = model->construct_pbc;
*/
...
{
int i, num_atoms, periodic;
double version;

/* record 1 - version */
i = READ_RECORD;
i = fread(&version, sizeof(version), 1, fp);
...
i = READ_RECORD;
/* # of atoms + shells */
i = fread(&num_atoms, sizeof(num_atoms), 1, fp);
if (model->trj_swap)
  swap_bytes(&num_atoms, sizeof(num_atoms));
/* dimension */
i = fread(&periodic, sizeof(periodic), 1, fp);
if (model->trj_swap)
...
/* get cell vectors */
  n = READ_RECORD;
  n = fread(cell, sizeof(double), 9, fp);
  n = READ_RECORD;
...
/* calculate nstrains [0, 6] */
  ...
  n = fread(velc, sizeof(double), j, fp);
  n = READ_RECORD;
...
for (i=0 ; i<model->num_frames ; i++)
  {
  read_trj_frame(fp, model, TRUE);
  animate_frame_store(model);
  }
/* revert to original */
animate_frame_select(0, model);
...
/************************************/
/* CURRENT - minimal import routine */
/************************************/
gint gulp_output_import(gpointer import)
{
/* pass through */
return(0);
}
