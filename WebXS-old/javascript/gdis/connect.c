...
/* EXPERIMENTAL - solvation hack */
if (core1->molecule == -1)
  {
  core_list = g_slist_copy(model->cores);
  }
else
  {
  zone = zone_get(x1, model->zone_array);
  core_list = zone_area_cores(1, zone, model->zone_array);
  }

/* check separations and create bonds */
...
/* NB: this is only valid if core1 and core2 are in the same list */
/* for mdi, core1 is not in the same list as core2, as it is a new */
/* solvent molecule candidate ... so skip this test */
  if (core1->molecule != -1)
    if (core1 > core2)
      continue;
...
/*
  if (connect_bond_test(x2, r2cut, core1, core2, model))
    {
*/
/* do periodic image checks in all cases - should trap most cases */
...
/* update dependent drawing primitives */
...
/* clear PRUNED flag - used to mark atoms attached to a molecule */
/* unless atom is deleted, then ignore (prune) it */
  if (core->status & DELETED)
    core->status |= PRUNED;
  else
    core->status &= ~PRUNED;

/* record core order in list */
  core->atom_order = i++;
  }
