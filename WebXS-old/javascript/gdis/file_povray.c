...
/********************************************/
/* NB: this really bloats the pov file - should only do when necessary */
/* it's also part of Josh's (current broken) stuff */
...
/* FIXME - transparancy works, but takes sooo long - better way? */
/*
      fprintf(povfile," <%f,%f,%f,%f> } finish { Phong_Shiny } }\n",
                       c[0], c[1], c[2], sysenv.render.transmit);
*/
...
/**************************************************************************/
/* enumerate all spatial vertex colours and declare the unique RGB values */
/**************************************************************************/
void povray_declare_textures(FILE *fp, struct model_pak *model)
{
gint i;
gchar *id;
GList *item;
...
//textures = g_hash_table_new(g_str_hash, g_str_equal);
textures = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, NULL);
...
for (list1=model->spatial ; list1 ; list1=g_slist_next(list1))
  ...
  for (list2=spatial->list ; list2 ; list2=g_slist_next(list2))
    ...
    i = povray_rgb_encode(vertex->colour);

    id = g_strdup_printf("RGB_%d", i);

    g_hash_table_insert(textures, id, vertex->colour);
    }
  }

//printf("unique list:\n");
for (item=g_hash_table_get_keys(textures) ; item ; item=g_list_next(item))
  {
//  printf("+ [%s]\n", (gchar *) item->data);
  vertex = g_hash_table_lookup(textures, item->data);

  if (vertex)
/*
    fprintf(fp, "#declare %s = texture{pigment{color rgb <%f,%f,%f> } finish { Phong_Shiny } }\n", 
                 (gchar *) item->data, vertex->colour[0], vertex->colour[1], vertex->colour[2]);
*/
    fprintf(fp, "#declare %s = texture{pigment{color rgbt <%f,%f,%f,%f> } finish { Phong_Shiny } }\n", 
                 (gchar *) item->data, vertex->colour[0], vertex->colour[1], vertex->colour[2], sysenv.render.transmit);
  else
    fprintf(fp, "#declare %s = texture{pigment{color rgb <0.0,0.0,1.0> } finish { Phong_Shiny } }\n", 
                 (gchar *) item->data);
  }
...
/********************************/
void povray_rgb(gdouble *povray_rgb, gdouble *gdis_rgb, struct model_pak *model)
{
gint i;
gdouble g;
...
if (model->grayscale)
  {
  g = gdis_rgb[0] + gdis_rgb[1] + gdis_rgb[2];
  g *= 0.333333333;
  for (i=3 ; i-- ; )
    povray_rgb[i] = g / 65535.0;
  }
else
  {
  for (i=3 ; i-- ; )
    povray_rgb[i] = gdis_rgb[i] / 65535.0;
  }
}
...
/* texture declarations for mesh construct */
if (g_slist_length(data->spatial))
  {
//  write_povray_colour_textures(fp,data,style);
// NEW - rewrote so we only declare the ones we need
  povray_declare_textures(fp, data);
  }
...
for (list=data->cores ; list ; list=g_slist_next(list))
  ...
  povray_rgb(colour, core1->colour, data);
...
/* TODO - can we adjust this to get a wire frame sphere? */
        fprintf(fp,"texture{pigment{color rgb<%f,%f,%f>",colour[0],colour[1],colour[2]);  
...
for (i=0 ; i<RENDER_LAST ; i++)
  ...
  for (list=pipe_list[i] ; list ; list=g_slist_next(list))
    ...
    core = pipe->core;
    povray_rgb(colour, core->colour, data);
...
/* AXES */
/********/
/* FIXME - broken under new camera model */
if (0)
if (data->show_axes)
  {
/* origin */
...
if (VEC3MAGSQ(v3) < 0.1)
  continue;
    ...
    fprintf(fp," texture { pigment {color rgb<%f,%f,%f>} } }\n",
                sysenv.render.fg_colour[0], sysenv.render.fg_colour[1], sysenv.render.fg_colour[2]);
...
/*
      texture{pigment{color rgbt<%.2f,%.2f,%.2f,%.1f>}
      finish { Phong_Shiny } }
*/
