...
/***************************************/
/* NB: the list, but NOT the elements in the list should be freed */
#define DEBUG_FIND_UNIQUE 0
GSList *find_unique(gint mode, struct model_pak *model)
{
GList *item, *list;
GSList *sitem, *unique=NULL;
struct core_pak *core;
GHashTable *type_table;

/* setup hash table for element number v label */
switch(mode)
  {
  case ELEMENT:
...
/* enumerate all atoms */
for (sitem=model->cores ; sitem ; sitem=g_slist_next(sitem))
  {
  core = sitem->data;
  ...
  switch(mode)
    {
    case ELEMENT:
      g_hash_table_insert(type_table, GINT_TO_POINTER(core->atom_code), NULL);
      break;

    case LABEL_FF:
      g_hash_table_insert(type_table, core->atom_type, NULL);
      break;

    case LABEL:
      g_hash_table_insert(type_table, core->atom_label, NULL);
      break;
...
/* duplicate unique key list for return */
list  = g_hash_table_get_keys(type_table);
for (item=list ; item ; item=g_list_next(item))
  unique = g_slist_prepend(unique, item->data); 

g_hash_table_destroy(type_table);

#if DEBUG_FIND_UNIQUE
printf("Found: ");
for (sitem=unique ; sitem ; sitem=g_slist_next(sitem))
  {
  switch(mode)
    {
    case ELEMENT:
      printf("[%3d] ", GPOINTER_TO_INT(sitem->data));
      break;
    case LABEL:
    case LABEL_NORMAL:
    case LABEL_GHOST:
    case LABEL_FF:
      printf("[%s] ", (gchar *) sitem->data);
      break;
    }
  }
printf("\n");
...
if (!core->atom_label)
  core->atom_label = g_strdup(elem.symbol);
...
#if DEBUG_CALC_EMP
printf("[%s] core : charge=%f (running sum: %f)\n", core->atom_label, core->charge, qsum);
#endif
...
#if DEBUG_CALC_EMP
printf("[%s] shel : charge=%f (running sum: %f)\n", core->atom_label, shell->charge, qsum);
#endif
    }
  }
...
/*******************************************************/
/* refresh atom colour based on currrent colour scheme */
/*******************************************************/
void atom_colour_refresh(struct core_pak *core, struct model_pak *model)
{
atom_colour_scheme(model->colour_scheme, core, model);

render_pipe_colour_refresh(core, model);
}
