...
/*****************************************/
void defect_isotropic_compliance_get(gdouble *s, struct defect_pak *defect)
{
gint i;
gdouble d1, d2, o1;

if (defect->modulus < 0.0000001)
  defect->modulus = 0.0000001;

d1 = 1.0 / defect->modulus;
d2 = (1.0+defect->ratio) / defect->modulus;
o1 = -defect->ratio / defect->modulus;
...
/* acquire the elastic compliance matrix */
/*****************************************/
/* TODO - if we don't have GULP ... permit isotropic complience matrix with user input */
// what do we need? 
// either: young's modulus(E) + poisson's ratio(v) ... or
// bulk and shear modulus ... see:
// http://www.efunda.com/formulae/solid_mechanics/mat_mechanics/hooke_isotropic.cfm
// probably just use E & v ... but if desired, can cpnvert using:
...
if (task_fortran_exe(sysenv.gulp_path, full_inp, full_out))
  return(1);
...
/*************************************************/
/* do coordinate offsets for a screw dislocation */
/*************************************************/
#define DEBUG_DEFECT_SCREW 0
...
if (fabs(r44) < FRACTION_TOLERANCE || fabs(r55) < FRACTION_TOLERANCE)
  {
  gui_text_show(ERROR, "Bad compliance matrix.\n"); 
  return;
  }
...
/* NEW - setup defect compliance matrix */
if (defect->isotropic)
  defect_isotropic_compliance_get(s, defect);
else
  memcpy(s, defect->s, 36*sizeof(gdouble));
...
if (defect_compliance_get(s, plane, source))
  {
  gui_text_show(ERROR, "Failed to compute compliance tensor.\n");
  return;
  }
else
  {
...
import = import_new();
text = g_build_filename(sysenv.cwd, "screw_defect", NULL);
...
printf("creating SCREW dislocation...\n");
  ...
  {
  gui_text_show(ERROR, "Edge dislocations not implemented yet.\n");
  }
