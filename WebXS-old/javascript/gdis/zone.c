...
/* FIXME - core dumps here where i is some stupidly large number */
...
/* sanity check on division sizes */
for (i=3; i--; )
  {
  if (div[i] < 1)
    div[i] = 1;

/* FIXME - base this on # cores? */
/* FIXME - corrupt (or big) distances can blow this out */
/* FIXME - hard limit currently is 1000? see zone_area_cores() */
  if (div[i] > 50)
    div[i] = 50;
...
for (k=0 ; k<div[2] ; k++)
  ...
  for (j=0 ; j<div[1] ; j++)
    ...
    for (i=0 ; i<div[0] ; i++)
      {
// NEW - a straight forward increment should do the same thing
/*
      zi = k*div[1]*div[0]; 
      zi += j*div[0]; 
      zi += i;
...
model->zone_array = zone_make(5.0, model);
}
...
/************************/
/* add a core to a zone */
/************************/
void zone_core_add(struct core_pak *core, gpointer data)
{
struct zone_pak *zone=data;
