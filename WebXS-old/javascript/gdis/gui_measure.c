...
/* info on bonds */
/*****************/
void measure_bond_event(gint x, gint y, struct model_pak *model)
{
gpointer measure;
...
/* info on dists */
/*****************/
void measure_dist_event(struct core_pak *current, struct model_pak *model)
{
gpointer measure;
GSList *list;
struct core_pak *core[2];
...
/* info on angles */
/******************/
void measure_angle_event(struct core_pak *current, struct model_pak *model)
{
gint n;
gpointer measure;
...
switch (n)
  ...
  case 2:
// select_add_core prepends to list -> reverse order
    core[0] = g_slist_nth_data(model->selection, 1);
    core[1] = g_slist_nth_data(model->selection, 0);
    core[2] = current;
...
/****************************/
void measure_torsion_event(struct core_pak *current, struct model_pak *model)
{
gint n;
gpointer measure;
...
switch (n)
  ...
  case 3:
    core[0] = g_slist_nth_data(model->selection, 0);
    core[1] = g_slist_nth_data(model->selection, 1);
    core[2] = g_slist_nth_data(model->selection, 2);
    core[3] = current;
...
gint i;
gpointer dialog;
gchar *titles[] = {" Structure ", " Constituent atoms ", " Value "};
GSList *list;
...
gui_button("Print", meas_dump_all, NULL, hbox, TT);

/* terminating buttons */
gui_stock_button(GTK_STOCK_HELP, gui_help_show, "Measurements", GTK_DIALOG(window)->action_area);
