...
gdouble tmat[9];
gdouble tvec[3];
gint edit_nanotube_overlay=FALSE;
...
GtkWidget *apd_sof, *apd_growth, *apd_region, *apd_translate;
...
model = gui_tree_append_model("new_model", NULL);
...
elem = gtk_entry_get_text(GTK_ENTRY(apd_label));

/* add the atom to the model */
core = core_new(elem, NULL, data);
if (!core)
  return;
...
/* check if its the unit vector (ie likely unchanged from default) */
  for (i=model->periodic ; i<n ; i++)
    ...
    if (d < FRACTION_TOLERANCE)
      {
      if (gui_popup_confirm("Suspicious transformation matrix repeat vectors, continue?") != 1)
        return;
      else
        break;
      }
...
/* NEW - enforce default region (can cause problems with surfaces) */
for (list=model->cores ; list ; list=g_slist_next(list))
  {
  core = list->data;
  core->region = 0;
...
{
gint i, j, k, n, m, d, dr, np, mp, graft=FALSE;
...
/* attempt to get model if overlay desired */
if (edit_nanotube_overlay)
  {
  model = tree_model_get();
  import = import_find(IMPORT_MODEL, model);
//printf("overlay, model [%p], import [%p]\n", model, import);
  if (!model || !import)
    {
    gui_popup_message("Failed to find existing import/model for overlay.");
    return;
    }
  }
else
...
/* create new */
  import = import_new();
  text = g_build_filename(sysenv.cwd, "nanotube", NULL);
  import_fullpath_set(text, import);
  g_free(text);
...
/* setup */
/* FIXME - workaround - not sure why we get so many duplicates ... */
coords_remove_duplicates(model);
...
/* if new import (and model) add to tree */
if (graft)
  gui_tree_append_import(import);

/* enforce target model is selected */
tree_select_model(model);
...
if (model)
  ...
  model_prep(model);
/* restore camera so viewing angle is the same (model_prep reverts to default) */
  camera_copy(model->camera, camera);
  g_free(camera);
  }
...
/********************************************/
/* project level model build/create options */
/********************************************/
void gui_nanotube_builder(GtkWidget *box)
{
...
spin = gui_spin_new(&edit_chirality[0], 0.0, 99.0, 1.0);
gtk_table_attach_defaults(GTK_TABLE(table),spin,1,2,1,2);

spin = gui_spin_new(&edit_chirality[1], 0.0, 99.0, 1.0);
gtk_table_attach_defaults(GTK_TABLE(table),spin,2,3,1,2);

/* basis atoms */
label = gtk_label_new("Basis");
...
entry = gui_text_entry(NULL, &edit_basis[0], TRUE, TRUE, NULL);
gtk_table_attach_defaults(GTK_TABLE(table),entry,1,2,0,1);
/* HACK - restrict entry width so it doesnt take up too much room */
gtk_entry_set_width_chars(GTK_ENTRY(entry), 4);

entry = gui_text_entry(NULL, &edit_basis[1], TRUE, TRUE, NULL);
gtk_table_attach_defaults(GTK_TABLE(table),entry,2,3,0,1);
/* HACK - restrict entry width so it doesnt take up too much room */
gtk_entry_set_width_chars(GTK_ENTRY(entry), 4);

/* characteristic length */
spin = gui_spin_new(&edit_length, 0.1, 5.0, 0.05);
gtk_table_attach_defaults(GTK_TABLE(table),spin,3,4,0,1);

gui_checkbox("Overlay ", &edit_nanotube_overlay, vbox);

gui_button_x("Create nanotube ", edit_nanotube_new, NULL, vbox);
}

/* CURRENT - the new model builder tab */
/* deprec - back to old school */
void gui_builder_widget(GtkWidget *box)
{
GtkWidget *hbox, *vbox, *left, *right;
...
/* new model */
/* general */
vbox = gui_frame_vbox("General", FALSE, FALSE, left);
gui_button_x("Create new model", edit_model_create, NULL, vbox);

/* nano tube */
gui_nanotube_builder(left);

/* library */
gui_library_window(right);
}

/**********************/
/* model builder page */
/**********************/
/* CURRENT - more of a structural/contraints page */
void edit_structure_page(GtkWidget *box)
{
GtkWidget *vbox;
...
/* Frame */
vbox = gui_frame_vbox("Constraints", FALSE, FALSE, box);
...
/* Surface options */
vbox = gui_frame_vbox("Surface", FALSE, FALSE, box);
gui_button_x("Move selection to top", region_move, (gpointer) UP, vbox);
gui_button_x("Move selection to bottom", region_move, (gpointer) DOWN, vbox);
...
if (model)
  spatial_destroy_by_label(label, model);
...
{
spatial_destroy(spatial_selected, tree_model_get());
sysenv.refresh_dialog=TRUE;
redraw_canvas(SINGLE);
}
...
g_free(dest_model->filename);
dest_model->filename = g_strdup("new model");
...
void sculpt_model_create(gdouble length, struct surface_pak *surface, struct model_pak *model)
{
gint i, t[3], limit[3];
...
if (model->periodic != 3)
  {
  printf("Source model is not 3D periodic.\n");
  return;
  }
if (!surface->planes)
  {
  printf("No cleavage planes supplied.\n");
  return;
  }
#if DEBUG_SCULPT_CREATE
else
  printf("Planes: %d\n", g_slist_length(surface->planes));
#endif
...
for (i=0 ; i<model->periodic ; i++)
  limit[i] = 1 + length/model->pbc[i];

/* init destination model for sculpture */
dest = model_new();
if (!dest)
  {
  printf("Failed to allocate for new model.\n");
  return;
  }
...
rmin = G_MAXDOUBLE;
//for (plist=surface->planes ; plist ; plist=g_slist_next(plist))
...
scale = 0.5*length/rmin;

#if DEBUG_SCULPT_CREATE 
printf("rmin = %f, scale = %f\n", rmin, scale);
#endif
...
dest->morph_label=TRUE;

gui_tree_append_model("nuclei", dest);
...
/* core updates */
  ...
  case CORE_SOF:
    text = gtk_entry_get_text(GTK_ENTRY(apd_sof));
    for (list=model->selection ; list ; list=g_slist_next(list))
      {
      core = list->data;
      core->sof = str_to_float(text);
      }
    break;
...
/* REFRESH */
      atom_colour_refresh(apd_core, model);
      gui_refresh(GUI_ACTIVE);
      }
    break;
  ...
  case CORE_SOF:
    text = gtk_entry_get_text(GTK_ENTRY(apd_sof));
    apd_core->sof = str_to_float(text);
    break;
...
gchar *element, *label, *type, *charge, *x, *y, *z, *sof, *growth, *region, *translate;
struct core_pak *core;
struct model_pak *model;
GSList *list;

/* TODO - better check for model editing dialog */
if (!GTK_IS_WIDGET(apd_label))
  return;
...
/* data available */
  ...
  sof = g_strdup_printf("%.4f", core->sof);
...
gtk_entry_set_text(GTK_ENTRY(apd_sof), sof);
...
entry = gtk_entry_new();
gtk_entry_set_text(GTK_ENTRY(entry), "Occupancy");
gtk_entry_set_editable(GTK_ENTRY(entry), FALSE);
gtk_box_pack_start(GTK_BOX(vbox), entry, FALSE, FALSE, 0);
...
gtk_entry_set_text(GTK_ENTRY(apd_label), "H");
...
apd_sof = gtk_entry_new();
gtk_box_pack_start(GTK_BOX(vbox), apd_sof, FALSE, FALSE, 0);
...
g_signal_connect(GTK_OBJECT(apd_sof), "activate",
                 GTK_SIGNAL_FUNC(atom_properties_change), GINT_TO_POINTER(CORE_SOF));
...
// FIXME - test (adds to selected) might not be as expected */
gui_button_x("Add shells", edit_shells_add, NULL, vbox);
gui_button_x("Add hydrogens", gtk_mode_switch, (gpointer) ATOM_H_ADD, vbox);
...
/* editing button set */
vbox = gui_frame_vbox(NULL, TRUE, TRUE, box);
gui_button_x("Rescale model", gui_view_default, NULL, vbox);
...
notebook = gui_notebook_new(vbox);

/* page */
page = gtk_vbox_new(FALSE,0);
gui_notebook_page_append(notebook, "Molecular", page);
gui_edit_widget(page);

/* page */
page = gtk_vbox_new(FALSE,0);
gui_notebook_page_append(notebook, "Nanotubes", page);
gui_nanotube_builder(page);

/* page */
page = gtk_vbox_new(FALSE,0);
gui_notebook_page_append(notebook, "Structural", page);
edit_structure_page(page);

/* page */
page = gtk_vbox_new(FALSE,0);
gui_notebook_page_append(notebook, "Spatials", page);
spatial_page(page);

/* page */
page = gtk_vbox_new(FALSE,0);
gui_notebook_page_append(notebook, "Transformations", page);
trans_page(page);

/* page */
page = gtk_vbox_new(FALSE,0);
gui_notebook_page_append(notebook, "Library", page);
gui_library_window(page);

/* terminating buttons */
gui_stock_button(GTK_STOCK_HELP, gui_help_show, "Model Editing", GTK_DIALOG(window)->action_area);
