...
/*
 blanket catch all so models aren't left in a mode that requires
 info from the dialog (eg add atom mode) 
*/
...
file_data = file_type_from_label(tmp);
if (file_data)
  {
...
/*************************************************/
void gui_dialog_file_save_as_action(GtkWidget *w, gpointer data)
{
gchar *fullpath, *message;
gpointer import;
GSList *item, *list;
GtkEntry *entry=data;

fullpath = g_build_filename(sysenv.cwd, gtk_entry_get_text(GTK_ENTRY(entry)), NULL);

import = tree_import_get();
if (!import)
  gui_text_show(ERROR, "Nothing to save.\n");
else
  {
/* NEW - force coordinate save based on check box */
  list = import_object_list_get(IMPORT_MODEL, import);
  for (item=list ; item ; item=g_slist_next(item))
    {
    struct model_pak *model = item->data;
    model->fractional = 1 - file_cartesian;
//printf("model=%p, fractional=%d\n", model, model->fractional);
    }

/* save the file */
  if (import_save_as(fullpath, import))
    {
    gui_text_show(ERROR, "Save failed. Check filename, extension type, and directory permissions.\n"); 
    }
  else
    {
    message = g_strdup_printf("File saved as: %s\n", fullpath);
    gui_text_show(STANDARD, message);
    g_free(message);
    dialog_destroy_type(FILE_SAVE_AS);
    }
  }
}
...
/* file save as dialog */
/***********************/
void gui_dialog_file_save_as(void)
{
gpointer dialog, import;
GtkWidget *window, *swin, *hbox, *label;
GtkCellRenderer *renderer;
GtkTreeViewColumn *column;
GtkTreeSelection *select;

/* get a dialog if possible */
dialog = dialog_request(FILE_SAVE_AS, "File Save As", NULL, NULL, NULL);
if (!dialog)
  return;
window = dialog_window(dialog);

/* make and set up the dialog window */
//gtk_window_set_title(GTK_WINDOW(window), "File Save as");
...
/* TODO - option to save as cartesian or fractional coords */
hbox = gtk_hbox_new(FALSE,0);
gtk_box_pack_start(GTK_BOX(GTK_DIALOG(window)->vbox),hbox,FALSE,FALSE,0);
gui_checkbox("Save as cartesian", &file_cartesian, hbox);
...
/* default to import filename */
import = tree_import_get();
if (import)
  gtk_entry_set_text(GTK_ENTRY(file_name), import_filename(import));

/* hook a <CR> event to the load action */
g_signal_connect(GTK_OBJECT(file_name), "activate", 
                 GTK_SIGNAL_FUNC(gui_dialog_file_save_as_action), file_name);

gui_stock_button(GTK_STOCK_SAVE, gui_dialog_file_save_as_action, file_name, 
