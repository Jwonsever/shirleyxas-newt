...
GtkWidget *surface_miller[3];
GtkWidget *surface_region[2];
GtkWidget *surface_shift;
GtkWidget *gui_surface_abc;
GtkWidget *gui_surface_aps;
GtkWidget *gui_surface_oec;
gpointer gui_surface_model_list=NULL;
//GtkWidget *surf_hkl_family;
gint all_planes = FALSE;
gdouble rank_value = 10.0;
gchar *morphology_type=NULL;
struct model_pak surfdata;
gchar *titles[] = {"hkl/shift", "Dhkl/depth",
                   "Esurf (u)", "Eatt (u)", "Esurf (r)", "Eatt (r)",
                   "dipole  ", "gnorm  "};

gpointer surface_project=NULL;
...
/**************************************/
/* convenience pointer initialization */
/**************************************/
gint gui_surface_init(gpointer **project, struct surface_pak **surface, struct model_pak **model)
{
gint n;
GSList *list;
...
/* get surface */
*surface = project_data_get(PROJECT_SURFACES, *project);
if (!*surface)
  return(FALSE);

//printf("surface = %p\n", *surface);

n = gui_pd_index_get(gui_surface_model_list);

/* if -1 -> use the 1st model in the list */
if (n == -1)
  n=0;

//printf("Active model index = %d\n", n);

list = project_model_list_get(*project);
...
/* get model */
/*
*model = gui_surface_model(*project);
if (!*model)
  {
  printf("No valid unit cell found in project.\n");
  return(FALSE);
  }
*/
...
/* NEW - transfer relevent surface requirements (from GUI) to model */
(*model)->surface.ignore_bonding = (*surface)->ignore_bonding;
(*model)->surface.include_polar = (*surface)->include_polar;

return(TRUE);
}

/**********************************/
/* push surface pak values to GUI */
/**********************************/
void gui_surface_push(struct surface_pak *surface)
{
if (!surface)
  return;

if (surface->ignore_bonding)
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(gui_surface_abc), TRUE);
else
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(gui_surface_abc), FALSE);

if (surface->include_polar)
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(gui_surface_aps), TRUE);
else
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(gui_surface_aps), FALSE);
}

/********************************************************/
/* pull widget values from GUI and populate surface pak */
/********************************************************/
void gui_surface_pull(GtkWidget *w, gpointer dummy)
{
gpointer *project;
struct model_pak *model;
struct surface_pak *surface;

if (!gui_surface_init(&project, &surface, &model))
  return;

if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(gui_surface_abc)))
  surface->ignore_bonding = TRUE;
else
  surface->ignore_bonding = FALSE;

if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(gui_surface_aps)))
  surface->include_polar = TRUE;
else
  surface->include_polar = FALSE;
}

/*****************************/
/* add project shift to tree */
/*****************************/
void gui_surface_graft_shift(GtkTreeIter parent, gpointer plane, struct shift_pak *shift)
{
gchar *title;
GtkTreeIter child;
...
/*****************************/
/* add project plane to tree */
/*****************************/
void gui_surface_graft_plane(struct plane_pak *plane)
{
gchar *title, *depth;
GSList *item;
GtkTreeIter root;
...
gtk_tree_store_set(surf_tree_store, &root, SURF_TITLE, title, SURF_REGIONS, depth,
                                           SURF_PLANE, plane, SURF_SHIFT, NULL, -1);
g_free(title);
g_free(depth);
...
/* add all shifts */
for (item=plane->shifts ; item ; item=g_slist_next(item))
  {
  shift = item->data;
  gui_surface_graft_shift(root, plane, shift);
  }
...
/**********************************/
/* add project plane list to tree */
/**********************************/
void gui_surface_graft_plane_list(GSList *list)
{
GSList *item;
...
for (item=list ; item ; item=g_slist_next(item))
  ...
  plane = item->data;
  if (plane->primary)
    gui_surface_graft_plane(plane);
  }
}

/********************************************************/
/* helper to get model the current project is acting on */
/********************************************************/
gpointer gui_surface_model(gpointer project)
{
GSList *item1, *list1, *item2, *list2;
...
for (item1=list1 ; item1 ; item1=g_slist_next(item1))
  ...
  for (item2=list2 ; item2 ; item2=g_slist_next(item2))
    {
    struct model_pak *model = item2->data;
    if (model->periodic == 3)
      return(model);
    }
  }
...
/*********************************************/
/* add plane with only current dialog values */
/*********************************************/
void gui_surface_plane_add(void)
{
gdouble len;
gpointer *project;
struct model_pak *source, *model;
struct surface_pak *surface;
struct plane_pak *plane;
struct shift_pak *shift;

if (!gui_surface_init(&project, &surface, &source))
  return;

//printf(">>> project = %p, surface = %p, source = %p\n", project, surface, source);
len = VEC3MAGSQ(surface->miller);
if (len < FRACTION_TOLERANCE)
  {
  gui_text_show(ERROR, "Bad miller indices.\n");
  return;
  }
...
model = model_new();
model->surface.shift = shift->shift;
model->surface.region[0] = 1;
model->surface.region[1] = 0;

/* TODO - does the plane already exist? */
plane = surface_hkl_find(surface->miller, surface->planes, source);
if (plane)
  {
...
/* create new plane with current hkl */
  plane = plane_new(surface->miller, source);
/* append new plane to model */
  surface->planes = g_slist_append(surface->planes, plane);
  }

if (plane)
  { 
/* write the plane index back - may be altered by symmetry */
  ARR3SET(surface->miller, plane->index);
  generate_surface(source, model);

  shift->dipole = model->gulp.sdipole;
  shift->dipole_computed = TRUE;

/* append shift to plane */
  plane->shifts = g_slist_append(plane->shifts, shift);
  }

model_free(model);

gui_surface_update(project);
}

/************************************/
/* add plane with only valid shifts */
/************************************/
void gui_surface_shifts_add(void)
{
gdouble len;
gpointer *project;
struct model_pak *source;
struct surface_pak *surface;
struct plane_pak *plane, *new;
...
new = plane_new(surface->miller, source);

plane = surface_hkl_find(new->m, surface->planes, source);
if (plane)
  {
  g_free(new);
  new = plane;
  ...
  surface->planes = g_slist_append(surface->planes, new);

calc_valid_shifts(source, new);
...
/***************************************************/
void gui_morph_build(GtkWidget *w, gpointer dummy)
{
gdouble scale;
gpointer *project;
GSList *item1, *item2;
...
//surface_symmetry_generate(surface, model);
//model->planes = surface->planes;
/* TODO - get and set surface->morph_type */

/* generate vertices */
morph_build(surface, model);

/* FIXME - post processing of spatial sizes */
/* FIXME - not sure why it's not scaled right as is ... */
/*
scale = sqrt(model->ilatmat[0]*model->ilatmat[0] + model->ilatmat[4]*model->ilatmat[4] + model->ilatmat[8]*model->ilatmat[8]);
printf("ilat len = %f\n", scale);
scale = VEC3MAG(model->pbc);
printf("diag len = %f\n", scale);
*/
/* this is wrong for zircon.gin (should be 11) */
scale = model->volume / 15.0;

/* post process hack */
for (item1=model->spatial ; item1 ; item1=g_slist_next(item1))
  {
  spatial = item1->data;
  if (spatial->type != SPATIAL_MORPHOLOGY)
    continue;
  for (item2=spatial->list ; item2 ; item2=g_slist_next(item2))
    {
    struct vec_pak *vec = item2->data;

    VEC3MUL(vec->x, scale);
    }
  }

model->morph_label = TRUE;
...
import = import_new();
import_filename_set("new_habit", import);
project_imports_add(project, import);

/* read morphology file in & compute hull */
data = model_new();
import_object_add(IMPORT_MODEL, data, import);
...
if (!status)
  {
//  tree_model_add(data);
  gui_tree_append(project, TREE_IMPORT, import);
  }
...
/********************************/
void gui_morph_sculpt(GtkWidget *w, gpointer data)
{
gdouble length;
gpointer *project;
struct model_pak *model;
...
length = SPIN_FVAL(GTK_SPIN_BUTTON(data));

sculpt_model_create(length, surface, model);
}
...
/******************************/
void exec_ecalc_task(gpointer data)
{
GSList *input, *output;
struct grid_pak *job=data;

input = job->local_upload_list;
output = job->remote_file_list;

if (input && output)
  task_fortran_exe(sysenv.gulp_path, input->data, output->data);
}
...
void proc_ecalc_task(gpointer data)
{
GSList *output;
struct shift_pak *sdata;
struct grid_pak *job=data;
...
/* read gulp file */
output = job->remote_file_list;
if (read_gulp_output(output->data, model))
  printf("proc_ecalc_task() error: read failed.\n");

/* TODO - check this shift still exists */
sdata = job->parent;
if (sdata)
  {
...
void new_ecalc_task(struct model_pak *model,
                    ...
                    struct shift_pak *sdata)
{
gint r1size, save_method;
gchar *input, *output;
gpointer parent, import, config;
GSList *list;
struct model_pak *surface;
struct core_pak *core;
struct grid_pak *job;
...
if (!sysenv.gulp_path)
  {
  gui_popup_message("GULP executable required for this operation.");
  return;
  }
...
import = import_new();
import_object_add(IMPORT_MODEL, surface, import);

/* build the surface */
surface->id = GULP;
surface->surface.model = NULL;
surface->surface.shift = sdata->shift;
surface->surface.region[0] = sdata->region[0];
surface->surface.region[1] = sdata->region[1];
ARR3SET(surface->surface.miller, pdata->index);
generate_surface(model, surface);

/* get GULP config of source import */
parent = gui_tree_parent_object(model);
g_assert(parent != NULL);
config = import_config_get(GULP, parent);
g_assert(config != NULL);
gulp = config_data(config);
g_assert(gulp != NULL);

/* enforce conv for surfaces */
save_method = gulp->method;
gulp->method = CONV;


if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(gui_surface_oec)))
  {
printf("Forcing opti...\n");
gulp->run = E_OPTIMIZE;
  }
  ...
  {
printf("Forcing single...\n");
gulp->run = E_SINGLE;
  }


/* add GULP config to new surface */
import_object_add(IMPORT_CONFIG, config, import);

/* compute surface bulk energy */
gulp->sbulkenergy = gulp->energy;
gulp->sbulkenergy /= (gdouble) model->num_atoms;
r1size=0;
for (list=surface->cores ; list ; list=g_slist_next(list))
  {
  core = list->data;
...
if (r1size)
  gulp->sbulkenergy *= (gdouble) r1size;
else
  printf("Warning: empty region 1.\n");

/* FIXME - cheat, by tacking the shift to update on the *planes* slist */
/*
g_assert(surface->planes == NULL);
surface->planes = g_slist_append(surface->planes, sdata);
*/
...
/* CURRENT - record the shift to update here ... */
job->parent = sdata;

input = g_build_filename(job->remote_cwd, "dummy.gin", NULL);
output = g_build_filename(job->remote_cwd, "dummy.got", NULL);

job->local_upload_list = g_slist_prepend(job->local_upload_list, input);
job->remote_file_list = g_slist_prepend(job->remote_file_list, output);

import_save_as(input, import);
task_new("Energy", &exec_ecalc_task, job, &proc_ecalc_task, job, NULL);

/* dereference GULP object from surface */
import_object_remove(IMPORT_CONFIG, config, import);
gulp->method = save_method;
...
gint surf_conv(struct surface_pak *surface, gpointer gulp_config, gint type)
{
gint n, flag, relax, region, r1size, status, save_method;
gdouble sbe, de, old_energy;
gchar *inp, *out, *full_inp, *full_out;
GSList *list;
gpointer import;
struct model_pak *surf, *model;
struct core_pak *core;
struct gulp_pak *gulp;
/*
struct plane_pak *plane;
...
/* create "wrapper" import */
import = import_new();
import_object_add(IMPORT_MODEL, surf, import);
...
/* add GULP config to new surface */
import_object_add(IMPORT_CONFIG, gulp_config, import);

ARR3SET(surf->surface.miller, surface->miller);
surf->surface.region[0] = surface->region[0];
surf->surface.region[1] = surface->region[1];
surf->surface.shift = surface->shift;
surf->surface.converge_eatt = surface->converge_eatt;
surf->sginfo.lookup = FALSE;

/* which energy to converge */
if (gulp->run == E_OPTIMIZE)
  relax = 1;
else
  relax = 0;
...
// surf->gulp.sbulkenergy = sbe * (gdouble) r1size;
 gulp->sbulkenergy = sbe * (gdouble) r1size;
...
/* create appropriate name */
  ...
  full_inp = g_build_filename(surface->cwd, inp, NULL);
  full_out = g_build_filename(surface->cwd, out, NULL);
...
/* command cycle: write input, execute, read output */
  import_save_as(full_inp, import);
  task_fortran_exe(sysenv.gulp_path, full_inp, full_out);
  read_gulp_output(full_out, surf);
...
/* bad convergence checks */
  if (n > 10 && de > 0.1)
    {
    printf("ERROR: failed convergence cycle.\n");
    status = 1;
    break;
    }
...
surface->region[region] = surf->surface.region[region];
surface->eatt[region] = surf->gulp.eatt[region];
surface->esurf[region] = surf->gulp.esurf[region];
surface->gnorm = surf->gulp.gnorm;

/* deref */
import_object_remove(IMPORT_CONFIG, gulp_config, import);
gulp->method = save_method;
...
void exec_regcon_task(struct surface_pak *surface, struct task_pak *task)
{
gint m, r;
gchar *tmp;
gpointer import, config;
struct model_pak *model;
struct gulp_pak *gulp;
FILE *fp;
...
tmp = g_build_filename(surface->cwd, "source.gin", NULL);

printf("Importing source cell: %s\n", tmp);

import = import_file(tmp, NULL);
if (!import)
  return;
...
/* set current surface convergence source model */
surface->model = model;

/* get GULP config and set desired values */
config = import_config_get(GULP, import);
if (!config)
  return;
...
m = GCD(surface->miller[0], GCD(surface->miller[1], surface->miller[2]));

if (surface->dspacing < MIN_THICKNESS)
  {
/* FIXME - what to do if dhkl is very small or even zero */
  r = 1 + (gint) (MIN_THICKNESS / (m*surface->dspacing));

  if (r > surface->region[0])
    surface->region[0] = r;
  if (r > surface->region[1])
    surface->region[1] = r;
  }

fprintf(fp, "----------------------------------------------\n");
fprintf(fp, "         Miller: %f %f %f \n", surface->miller[0] , surface->miller[1] , surface->miller[2]);
fprintf(fp, "           Dhkl: %f\n", surface->dspacing);
fprintf(fp, "          Shift: %f\n", surface->shift);
fprintf(fp, "Initial regions: %d , %d\n", (gint) surface->region[0], (gint) surface->region[1]);
fprintf(fp, " Lattice energy: %f\n", surface->totalenergy);
fprintf(fp, "    Convergence: ");
if (surface->converge_eatt)
...
if (model->surface.converge_r2)
  {
  if (surf_conv(surface, config, REGION2A))
    {
...
if (model->surface.converge_r1)
  {
  if (surf_conv(surface, config, REGION1A))
    {
...
/**************************************/
void proc_regcon_task(struct surface_pak *surface)
{
GSList *list;
struct shift_pak *shift;
...
printf("Final Esurf: %f , %f\n", surface->esurf[0], surface->esurf[1]);
printf("Final Eatt: %f , %f\n", surface->eatt[0], surface->eatt[1]);
printf("Final gnorm: %f\n", surface->gnorm);
*/

/* TODO - check the shift still exists */
list = surface->planes;
if (list)
  {
  shift = list->data;
  ...
  shift->region[0] = surface->region[0];
  shift->region[1] = surface->region[1];
  shift->esurf[0] = surface->esurf[0];
  shift->esurf[1] = surface->esurf[1];
  shift->eatt[0] = surface->eatt[0];
  shift->eatt[1] = surface->eatt[1];
  shift->gnorm = surface->gnorm;
  }

/* the data in this list isn't ours - so make sure surface_free() wont free it */
g_slist_free(surface->planes);
surface->planes = NULL;

/* update gui */
//gui_surface_update(tree_project_get());
gui_surface_widget_update();
...
/* TODO - fill out surface with details from dialog check boxes */
/* ie get from the surface pak of gui_surface_init() */

/* fill out selected plane/shift into surface struct */
ARR3SET(surface->miller, plane->index);
surface->region[0] = shift->region[0];
surface->region[1] = shift->region[1];
surface->shift = shift->shift;
surface->dspacing = plane->dhkl;
surface->planes = g_slist_prepend(surface->planes, shift);

/* create tmpdir, save model there */
surface->cwd = file_unused_directory();
tmp = g_build_filename(surface->cwd, "source.gin", NULL);
import = gui_tree_parent_object(model);

g_assert(import != NULL);

printf("Saving source unit cell: %s\n", tmp);

import_save_as(tmp, import);
...
/* TODO - usual warning if this is 0 */
surface->totalenergy = gulp->energy;

task_new("Regcon", &exec_regcon_task, surface, &proc_regcon_task, surface, NULL);
}
...
for (row=list ; row ; row=g_list_next(row))
  ...
  if (gtk_tree_model_get_iter(treemodel, &iter, row->data))
    {
    gtk_tree_model_get(treemodel, &iter, SURF_PLANE, &plane, SURF_SHIFT, &shift, -1);
...
{
/* FIXME - morph build signature changed */
/*
gpointer camera;
...
/*******************************/
/* delete planes and/or shifts */
/*******************************/
/* if shift=NULL => delete all shifts */
void gui_surface_delete(struct plane_pak *plane, struct shift_pak *shift)
{
gint n=1;
GSList *list;
gpointer *project;
...
/* NB: don't delete the reference plane until the end */
    ...
    if (facet_equiv(source, plane->index, pcomp->index))
      {
      surface->planes = g_slist_remove(surface->planes, pcomp);
      g_free(pcomp);
      n++;
      }
    }
/* remove main plane */
  surface->planes = g_slist_remove(surface->planes, plane);
  plane_free(plane);
  }
}
...
for (row=g_list_last(list) ; row ; row=g_list_previous(row))
  ...
  if (gtk_tree_model_get_iter(treemodel, &iter, row->data))
    {
    gtk_tree_model_get(treemodel, &iter, SURF_SHIFT, &shift, SURF_PLANE, &plane, -1);
    if (plane)
      {
      gtk_tree_store_remove(surf_tree_store, &iter);
      gui_surface_delete(plane, shift);
      }
    }
  }
...
/************************************************/
void gui_surface_add_ranked(GtkWidget *w, gpointer dummy)
{
GSList *item, *list;
gpointer *project;
struct plane_pak *plane;
...
list = get_ranked_faces((gint) rank_value, 0.0, source);
...
while (item)
  ...
  if (surface_hkl_find(plane->m, surface->planes, source))
    {
/* exists, remove */
...
surface->planes = g_slist_concat(surface->planes, list);
...
/*
printf("TODO - get rid of model refs...\n");
return;
*/
...
//GtkWidget *surface_shift;
    ...
    if (plane)
      {
gtk_spin_button_set_value(GTK_SPIN_BUTTON(surface_miller[0]), plane->index[0]);
gtk_spin_button_set_value(GTK_SPIN_BUTTON(surface_miller[1]), plane->index[1]);
gtk_spin_button_set_value(GTK_SPIN_BUTTON(surface_miller[2]), plane->index[2]);

      }
    if (shift)
      {
gtk_spin_button_set_value(GTK_SPIN_BUTTON(surface_shift), shift->shift);
gtk_spin_button_set_value(GTK_SPIN_BUTTON(surface_region[0]), shift->region[0]);
gtk_spin_button_set_value(GTK_SPIN_BUTTON(surface_region[1]), shift->region[1]);
      }
    }
  }
  }
...
/* temp storage of dialog values */
//struct surface_pak surface;

/**************************************************/
/* redraw surface panel with current active items */
/**************************************************/
void gui_surface_update(gpointer project)
{
GSList *list;
struct surface_pak *surface;
...
//printf("gui_surface_update(): project=%p, surface=%p\n", project, surface);

/* update source model */
list = project_model_name_list_get(project);
gui_pd_list_set(list, gui_surface_model_list);
free_slist(list);

gui_spin_modify(surface_miller[0], &surface->miller[0]);
gui_spin_modify(surface_miller[1], &surface->miller[1]);
gui_spin_modify(surface_miller[2], &surface->miller[2]);
gui_spin_modify(surface_region[0], &surface->region[0]);
gui_spin_modify(surface_region[1], &surface->region[1]);
gui_spin_modify(surface_shift, &surface->shift);

/* CURRENT - update with project planes list */
gtk_tree_store_clear(surf_tree_store);
gui_surface_graft_plane_list(surface->planes);

/* NEW - alternate push/pull approach */
gui_surface_push(surface);
}
...
{
gui_surface_update(sysenv.workspace);
}

/************************************/
/* surface creation/cut elimination */
/************************************/
void gui_surface_widget(GtkWidget *box)
{
gint i;
GSList *list;
GtkWidget *scr_win, *table;
GtkWidget *hbox1, *hbox, *vbox1, *vbox3, *vbox;
...
/*
if (data->id == MORPH)
  gtk_window_set_title(GTK_WINDOW(window), "Morphology display");
else
  {
...
gtk_box_pack_start(GTK_BOX(box), hbox1, TRUE, TRUE, PANEL_SPACING);
...
/* frame 1 - source model */
hbox = gui_frame_hbox(NULL, FALSE, FALSE, vbox);
label = gtk_label_new("Source model");
gtk_box_pack_start(GTK_BOX(hbox),label,FALSE,FALSE,0);
gui_surface_model_list = gui_pd_new(NULL, NULL, NULL, hbox);

/* frame 2 - miller etc */
vbox1 = gui_frame_vbox(NULL, FALSE, FALSE, vbox);
...
surface_miller[0] = gui_spin_new(NULL, -99, 99, 1);
surface_miller[1] = gui_spin_new(NULL, -99, 99, 1);
surface_miller[2] = gui_spin_new(NULL, -99, 99, 1);
surface_region[0] = gui_spin_new(NULL, 0, 99, 1);
surface_region[1] = gui_spin_new(NULL, 0, 99, 1);
surface_shift = gui_spin_new(NULL, 0.0, 1.0, 0.05);

gtk_table_attach_defaults(GTK_TABLE(table),surface_miller[0],1,2,0,1);
gtk_table_attach_defaults(GTK_TABLE(table),surface_miller[1],2,3,0,1);
gtk_table_attach_defaults(GTK_TABLE(table),surface_miller[2],3,4,0,1);

gtk_table_attach_defaults(GTK_TABLE(table),surface_shift,3,4,1,2);

//gtk_spin_button_set_digits(GTK_SPIN_BUTTON(spin), 4);

gtk_table_attach_defaults(GTK_TABLE(table),surface_region[0],2,3,2,3);
gtk_table_attach_defaults(GTK_TABLE(table),surface_region[1],3,4,2,3);

/* create this surface */
/*
button = gui_button(" Create ", cb_surf_create, (gpointer) data, NULL, TF);
gtk_table_attach_defaults(GTK_TABLE(table),button,3,4,3,4);
*/

/* frame 3 - adding a faces */
//vbox1 = gui_frame_vbox(NULL, FALSE, FALSE, vbox);

gtk_box_pack_start(GTK_BOX(vbox1), gtk_hseparator_new(), FALSE, FALSE, 0);


//gui_button_x("Create the current surface", cb_surf_create, data, vbox1);
gui_button_x("Add single cut", gui_surface_plane_add, NULL, vbox1);
gui_button_x("Add all allowed cuts", gui_surface_shifts_add, NULL, vbox1);
...
spin = gui_spin_new(&rank_value, 1.0, 50.0, 1.0);
gtk_box_pack_start(GTK_BOX(hbox), spin, FALSE, FALSE, 0);

gui_button_x(" Dhkl ranked faces", gui_surface_add_ranked, NULL, hbox);


gtk_box_pack_start(GTK_BOX(vbox1), gtk_hseparator_new(), FALSE, FALSE, 0);


  gui_surface_abc = gui_checkbox("Allow bond cleaving", NULL, vbox1);
  gui_surface_aps = gui_checkbox("Allow polar surfaces", NULL, vbox1);

  g_signal_connect(GTK_OBJECT(gui_surface_abc), "clicked",
                   GTK_SIGNAL_FUNC(gui_surface_pull), NULL);
  g_signal_connect(GTK_OBJECT(gui_surface_aps), "clicked",
                   GTK_SIGNAL_FUNC(gui_surface_pull), NULL);
...
/* frame 3 - morphology construction */
vbox1 = gui_frame_vbox(NULL, FALSE, FALSE, vbox);
...
*/
gui_button_x("Create morphology", gui_morph_build, NULL, vbox1);

hbox = gtk_hbox_new(FALSE, PANEL_SPACING);
gtk_box_pack_start(GTK_BOX(vbox1), hbox, TRUE, TRUE, 0);
spin = new_spinner("Create nuclei with size ", 5.0, 1000.0, 1.0, NULL, NULL, hbox);

//g_object_set_data(G_OBJECT(spin), "model", data);
gui_button_x(NULL, gui_morph_sculpt, spin, hbox);
...
list = NULL;
list = g_slist_append(list, "Dhkl");
list = g_slist_append(list, "Esurf (u)");
list = g_slist_append(list, "Esurf (r)");
list = g_slist_append(list, "Eatt (u)");
list = g_slist_append(list, "Eatt (r)");
list = g_slist_append(list, "Broken bonds");

/* FIXME - morph types apear to have a scaling problem ... */
gui_pd_new("Morphology type ", &morphology_type, list, hbox);


/* FIXME - hide until fixed */
/* frame 4 - for convergence check buttons */
/*
  vbox1 = gui_frame_vbox(NULL, FALSE, FALSE, vbox);

  gui_direct_check("Attachment energy based", &surface.converge_eatt, NULL, NULL, vbox1);
  gui_direct_check("Converge region 1", &surface.converge_r1, NULL, NULL, vbox1);
  gui_direct_check("Converge region 2", &surface.converge_r2, NULL, NULL, vbox1);
*/

/* frame 5 - for option check buttons */
/*
  vbox1 = gui_frame_vbox(NULL, FALSE, FALSE, vbox);
...
*/
  ...
  vbox1 = gui_frame_vbox(NULL, FALSE, FALSE, vbox);
  gui_surface_oec = gui_checkbox("Calculate relaxed energies", NULL, vbox1);


/*
  vbox1 = gui_frame_vbox(NULL, FALSE, FALSE, vbox);

  gui_direct_check("Allow polar surfaces", &surface.include_polar,
                     NULL, NULL, vbox1);
  gui_direct_check("Allow bond cleaving", &surface.ignore_bonding,
                     NULL, NULL, vbox1);
  gui_direct_check("Ignore model symmetry", &surface.ignore_symmetry,
                     NULL, NULL, vbox1);
  gui_direct_check("Preserve depth periodicity", &surface.true_cell,
                     NULL, NULL, vbox1);
  gui_direct_check("Preserve atom ordering", &surface.keep_atom_order,
                     NULL, NULL, vbox1);
*/

/* frame 6 - misc options */
/*
  hbox = gui_frame_hbox(NULL, FALSE, FALSE, vbox);
  gui_direct_spin("Surface dipole cutoff ", &surface.dipole_tolerance,
...
/* frame 1 - valid cut listing */
vbox = gui_frame_vbox(NULL, TRUE, TRUE, vbox1);
...
/* TODO - include as part of project */
//fn_graft_plane_list(data->planes, data);
...
*/
  ...
  gui_button("Remove", surf_prune_selected, NULL, hbox, TT);

  gui_button("Calculate", surf_task_selected, (gpointer) CALC_ENERGY, hbox, TT);
  gui_button("Converge", surf_task_selected, (gpointer) CONV_REGIONS, hbox, TT);
  gui_button("Build", surf_task_selected, (gpointer) MAKE_FACES, hbox, TT);
...
surface_project = sysenv.workspace;
if (!surface_project)
  return;

dialog = dialog_request(GENSURF, "Surface Builder", NULL, NULL, NULL);
if (!dialog)
  return;
...
gui_surface_widget(GTK_DIALOG(window)->vbox);

gui_surface_widget_update();

gui_stock_button(GTK_STOCK_HELP, gui_help_show, "Surface Builder",
                   GTK_DIALOG(window)->action_area);
