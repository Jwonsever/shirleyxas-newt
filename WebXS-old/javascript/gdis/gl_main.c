...
/* catch no camera (can happen on a bad/non-existant model init */
...
/* NB: if near distance is 0.0 it causes drawing problems */
  ...
  r *= camera->zoom;
  if (a > 1.0)
    glOrtho(-r*a, r*a, -r, r, -sysenv.rsize, 4.0*sysenv.rsize);
  else
    glOrtho(-r, r, -r/a, r/a, -sysenv.rsize, 4.0*sysenv.rsize);
  }
...
/***********************************/
/* count an atom's important bonds */
/***********************************/
gint gl_bond_count(struct core_pak *core)
{
gint count=0;
GSList *list;
...
/******************************/
void gl_print_window(const gchar *str, gint x, gint y, struct canvas_pak *canvas)
{
gdouble w[3];
...
#if DEBUG_CORE_PROXIMITY
printf(" : %d %f %f : ", sysenv.size, model->rmax, camera->zoom);
printf("dr = %d, tol = %d ", dr, itol);
#endif
...
if (!canvas)
  {
  printf("Error: model is not attached to a canvas.\n");
  return(NULL);
  }

/* default tolerance (canvas width squared) */
rmin = sysenv.size*sysenv.size;
...
/* NB: assumes > canvas width return if no match */
...
/***********************************/
/* fill out colour array for atoms */
/***********************************/
void gl_colour_get(gdouble *colour, struct core_pak *core, struct model_pak *model)
{
gdouble g;

ARR4SET(colour, core->colour);

if (model->grayscale)
  {
  g = colour[0] + colour[1] + colour[2];
  g *= 0.333333333; 
  VEC3SET(colour, g, g, g);
  }
...
/***************************/
/* draw atoms in line mode */
/***************************/
void gl_draw_atoms(gint type, struct model_pak *model)
{
gint i;
GSList *list, *ilist;
gdouble colour[4];
...
g_assert(type < RENDER_LAST);
g_assert(type >= 0);

VEC3SET(&x[0], -sysenv.render.ball_rad, 0.0, 0.0);
VEC3SET(&x[3], sysenv.render.ball_rad, 0.0, 0.0);
VEC3SET(&x[6], 0.0, 0.0, -sysenv.render.ball_rad);
VEC3SET(&x[9], 0.0, 0.0, sysenv.render.ball_rad);
VEC3SET(&x[12], 0.0, -sysenv.render.ball_rad, 0.0);
VEC3SET(&x[15], 0.0, sysenv.render.ball_rad, 0.0);

for (list=model->atom_list[type] ; list ; list=g_slist_next(list))
  {
  core = list->data;

  gl_colour_get(colour, core, model);
  glColor4dv(colour);

  ilist = NULL;
  do
    {
/* setup coordinates around atom */
    memcpy(rx, x, 18*sizeof(gdouble));
    for (i=0 ; i<6 ; i++)
      {
      ARR3ADD(&rx[3*i], core->rx);
      }
...
#if DEBUG_DRAW_CORES
printf("gl_draw_cores(): size=%d\n", g_slist_length(cores));
#endif
...
/* setup geometric primitive */
/* NEW - use reference overlay model (if exists) for init */
/*
if (model->overlay)
  gl_init_sphere(&sphere, model->overlay);
else
  gl_init_sphere(&sphere, model);
*/

//sphere_mesh = va_init_sphere(0.5, model->camera);
if (quality)
  q = 0.9 / sysenv.render.sphere_quality;
else
  q = 0.9;
...
sphere_mesh = va_init_sphere(q, model->camera);


// FIXME - sphere mesh CW (standard gdis is ccw)
glFrontFace(GL_CW);
...
va_draw_hemisphere(sphere_mesh, x, radius);
...
if (core)
  gl_colour_get(colour, core, data);
else
  {
  ARR3SET(colour, shel->colour);
  }

/* enforce transparancy */
colour[3] = 0.5;
...
cylinder_mesh = va_init_cylinder(q);
glFrontFace(GL_CW);
...
//        gl_draw_cylinder(v1, v2, sysenv.render.stick_rad, q);
        va_draw_cylinder(cylinder_mesh, v1, v2, sysenv.render.stick_rad);
...
const gchar *gl_mode_label(struct model_pak *model)
{
gint n;
static gchar label[MAX_LABEL_SIZE];
...
switch(model->mode)
  {
  case FREE:
/* TODO - mb if phonons - vibrating ... */
    if (model->animating)
      g_snprintf(label, MAX_LABEL_SIZE, "animating");
    else
      g_snprintf(label, MAX_LABEL_SIZE, "normal");
    break;
  case OVERLAY:
    g_snprintf(label, MAX_LABEL_SIZE, "overlay");
    break;
  case ATOM_ADD:
    g_snprintf(label, MAX_LABEL_SIZE, "add atoms");
    break;
  case ATOM_H_ADD:
    g_snprintf(label, MAX_LABEL_SIZE, "add hydrogens");
    break;
  case BOND_DELETE:
    g_snprintf(label, MAX_LABEL_SIZE, "delete bonds");
    break;
  case BOND_SINGLE:
    g_snprintf(label, MAX_LABEL_SIZE, "add single bonds (%1d/2)", n);
    break;
  case BOND_DOUBLE:
    g_snprintf(label, MAX_LABEL_SIZE, "add double bonds (%1d/2)", n);
    break;
  case BOND_TRIPLE:
    g_snprintf(label, MAX_LABEL_SIZE, "add triple bonds (%1d/2)", n);
    break;
  case BOND_INFO:
    g_snprintf(label, MAX_LABEL_SIZE, "bonds");
    break;
  case DIST_INFO:
    g_snprintf(label, MAX_LABEL_SIZE, "distance selection (%1d/2)", n);
    break;
  case ANGLE_INFO:
    g_snprintf(label, MAX_LABEL_SIZE, "angle selection (%1d/3)", n);
    break;
  case DIHEDRAL_INFO:
    g_snprintf(label, MAX_LABEL_SIZE, "dihedral selection (%1d/4)", n);
    break;
  case DEFINE_RIBBON:
    g_snprintf(label, MAX_LABEL_SIZE, "define ribbon");
    break;
  case DEFINE_VECTOR:
    g_snprintf(label, MAX_LABEL_SIZE, "define vector (%1d/2)", n);
    break;
  case DEFINE_PLANE:
    g_snprintf(label, MAX_LABEL_SIZE, "define plane (%1d/3)", n);
    break;
  case ANIMATING:
    g_snprintf(label, MAX_LABEL_SIZE, "Frame: %-4d", model->cur_frame);
    break;
  case RECORD:
    g_snprintf(label, MAX_LABEL_SIZE, "recording");
    break;
  case SELECT_FRAGMENT:
    g_snprintf(label, MAX_LABEL_SIZE, "select bonded atoms (%1d/2)", n);
    break;
  default:
    g_snprintf(label, MAX_LABEL_SIZE, "undefined");
    break;
  }
...
/* print mode */
mode = gl_mode_label(data);
gl_print_window(mode, canvas->x+canvas->width-gl_text_width(mode), sysenv.height-canvas->y-20, canvas);
...
if (data->show_frame_number)
  ...
  else if (data->animate_list)
    {
    text = g_strdup_printf("[%d:%d]", 1+data->animate_current, g_list_length(data->animate_list));
    gl_print_window(text, canvas->x+canvas->width-gl_text_width(text),
                          sysenv.height-canvas->y-canvas->height+40, canvas);
    g_free(text);
    }
...
else if (data->current_phonon > 0)
  {
/* NB: current_phonon starts at 1 */
//  text = g_strdup_printf("[%d:%d]", data->current_phonon, data->num_phonons);
  text = g_strdup_printf("%s cm-1", (gchar *) g_slist_nth_data(data->phonons, data->current_phonon-1));
...
gl_draw_cores(data->atom_list[RENDER_SOLID], data);
...
/* draw solid bonds */
glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
gl_draw_pipes(FALSE, data->pipe_list[0], data);
...
gl_draw_cores(data->atom_list[RENDER_WIRE], data);
...
// trouble is - this will blend front/back transparent surfaces...
...
/* current fix is to allow depth buffer writing */
glDepthMask(GL_TRUE);
...
data->atom_list[RENDER_GHOST] = g_slist_sort(data->atom_list[RENDER_GHOST], (gpointer) gl_depth_sort);
gl_draw_cores(data->atom_list[RENDER_GHOST], data);

/* ghost bonds */
/* NB: back to front ordering */
data->pipe_list[1] = render_sort_pipes(data->pipe_list[1]);
gl_draw_pipes(FALSE, data->pipe_list[1], data);
...
glDisable(GL_CULL_FACE);
gl_draw_pipes(FALSE, data->pipe_list[2], data);
glEnable(GL_CULL_FACE);

/* stick (line) bonds */
gl_draw_pipes(TRUE, data->pipe_list[3], data);


/* stick (line) atoms */
gl_draw_atoms(RENDER_LINE, data);
...
/* don't show axes/mode text for overlays */
if (!data->overlay)
  {
...
printf("increasing delay between redraw: %d\n", n);
    ...
    gdk_threads_add_timeout(n*25, (GSourceFunc) &gui_canvas_handler, NULL);
#else
...
//printf("refresh = %d\n", sysenv.refresh_canvas);

/* is there anything to draw on? */
glcontext = gtk_widget_get_gl_context(sysenv.glarea);
if (!glcontext)
  {
  printf("gl_canvas_refresh(): could not create OpenGL context - misconfiguration?\n");
  return(FALSE);
  }
...
/* CURRENT - return to single canvas to enable graphs */
...
/* CURRENT - experiment with multiple draws */
// TODO - more control - ie select which models to draw (not just all)
// and also change colouring mode to differentiate easier
      if (model->mode == OVERLAY)
        {
        for (list=sysenv.mal ; list ; list=g_slist_next(list))
          {
// TODO - if model != data -> disable mode text (axes?) etc drawing
          model = list->data;
    ...
    case TREE_GRAPH:
      gl_init_projection(canvas, NULL);
...
/* NEW - allow multiple (sequential) requests */
if (sysenv.refresh_canvas)
  {
/* NB: the following bug ONLY occurs on a particular OS-X version */
/* for reasons I don't understand, the memory address for sysenv.refresh_canvas  */
/* is corrupted after calling gl_canvas_refresh() so attempts to access will cause */
/* a crash - hence all adjustments must be done BEFORE calling gl_canvas_refresh() */
/* To see this - print the value of sysenv.refresh canvas before and after calling */
/* gl_canvas_refresh() ... it changes ... BUT the routine itself does not touch it */
/* Note that there seems to be nothing fundamentally wrong as in subsequent calls */
/* of this routine sysenv.refresh_canvas is restored ... it just seems to be a */
/* state/memory context problem only appearing on OS-X */
  sysenv.refresh_canvas--;
  if (sysenv.refresh_canvas < 0)
